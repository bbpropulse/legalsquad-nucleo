...
export function SquadSelector() {
  ...
  const invalidStates = useSquadStore((s) => s.invalidStates);
  const selectedSquad = useSquadStore((s) => s.selectedSquad);
  const selectSquad = useSquadStore((s) => s.selectSquad);

  // Ordem: squads ativos primeiro — e um squad com estado ilegível conta como
  // ativo, porque quase sempre é uma execução rodando com arquivo quebrado.
  const squadList = Array.from(squads.values()).sort((a, b) => {
    const aActive = activeStates.has(a.code) || invalidStates.has(a.code) ? 0 : 1;
    const bActive = activeStates.has(b.code) || invalidStates.has(b.code) ? 0 : 1;
      ...
      <div style={{ flex: 1, overflowY: "auto" }}>
        ...
        {squadList.map((squad) => (
          <SquadCard
            ...
            invalidReason={invalidStates.get(squad.code)?.reason}
