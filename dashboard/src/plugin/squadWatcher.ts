...
import { squadRoster } from "./roster";

function resolveSquadsDir(): string {
  if (process.env.LEGALSQUAD_PROJECT_ROOT) return path.resolve(process.env.LEGALSQUAD_PROJECT_ROOT, 'squads');
  const candidates = [
...
async function discoverSquads(squadsDir: string): Promise<SquadInfo[]> {
  ...
  for (const entry of entries) {
    ...
    const party = await fsp.readFile(path.join(squadsDir, entry.name, 'squad-party.csv'), 'utf8').catch(() => '');
    try {
      ...
      if (s) {
        squads.push({
          ...
          roster: squadRoster(party, s.agents),
        });
        continue;
      }
    } catch {
  ...
  return {
    ...
    configureServer(server: ViteDevServer) {
      ...
      function handleFileChange(filePath: string) {
        ...
        } else if (fileName === "squad.yaml" || fileName === "squad-party.csv") {
          filas.enqueue(squadName, async () => {
