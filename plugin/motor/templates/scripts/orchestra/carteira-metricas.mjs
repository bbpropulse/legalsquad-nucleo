...
import { metricasCarteira, pastasDeCasosDaCasa } from './carteira-consolidar.mjs';

const asJson = process.argv.includes('--json');
const casosDir = join(process.cwd(), 'acervo', 'casos');
// Casos também vivem fora de acervo/casos/ (Processos/<caso>/): ver pastasDeCasosDaCasa.
const m = metricasCarteira(casosDir, { pastasDeCasos: pastasDeCasosDaCasa(process.cwd()) });
...
if (asJson) {
  ...
  console.log(`  por fase: ${Object.entries(m.por_fase).filter(([, n]) => n > 0).map(([f, n]) => `${f}=${n}`).join(' · ') || 'nenhuma'}`);
