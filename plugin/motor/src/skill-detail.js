...
export function detailSkill(id, rootDir, options = {}) {
  ...
  const resultado = {
    ...
    quality_profile_source: metadata?.qualityProfileSource || '',
...
export function skillDetailCli(id, targetDir, values = {}) {
  ...
  if (resultado.uso) {
    ...
    console.log(`  uso local: ${u.ciclos} ciclo(s) em ${u.squads_distintos} squad(s); ${u.aprovacoes} APPROVE / ${u.rejeicoes} REJECT (${u.rejeicoes_dirigidas ?? 0} dirigida(s) à skill); último ${u.ultimo_uso}`);
  } else {
