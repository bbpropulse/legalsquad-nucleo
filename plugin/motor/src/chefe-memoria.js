...
function reindexar(dir) {
  const linhas = arquivosDeFato(dir).map((nome) => {
    ...
    return `| ${fato.data || 'sem data'} | ${fato.tipo} | ${escaparPipe(fato.titulo)} | [${nome}](${nome}) |`;
  });
...
export function memoriaCli(raiz, subcomando, opcoes = {}) {
  ...
  for (const f of fatos) console.log(`  ${f.data || 'sem data'}  ${String(f.tipo).padEnd(12)} ${f.titulo}`);
  return { success: true, fatos };
}
