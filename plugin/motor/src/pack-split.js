...
export function lerCorteDePacotes(raizConteudo) {
  ...
  return {
    ...
    // Prefixo dos ids de squad-modelo desta área. Os modelos viajam com caminho
    // literal (`squads/_modelos/<id>/`), sem namespace por área: duas áreas que
    // publiquem `apelacao` gravam o MESMO caminho no depósito, e o último pacote
    // aplicado apaga os arquivos do outro em silêncio (`pack-apply` só detecta
    // caminho repetido dentro do mesmo pacote). "Apelação", "embargos" e
    // "contestação" são nomes naturais em cível, previdenciário, trabalhista e
    // criminal ao mesmo tempo, então a colisão é certa. Declarado pelo curador:
    // `modelo_prefixo: prev` exige `prev-...` em todo modelo da área;
    // `modelo_prefixo: ""` é a área-base, que publica sem prefixo. A chave só é
    // exigida de quem TEM modelos (ver `modelosForaDoPrefixo`).
    modeloPrefixo: chavePresente(bruto, 'modelo_prefixo') ? (parseScalar(bruto, 'modelo_prefixo') || '') : null,
  };
}

/** Ids de squad-modelo que o prefixo declarado da área não cobre. */
export function modelosForaDoPrefixo(entidades, prefixo) {
  const ids = [...new Set(entidades
    .map((e) => /^squads\/_modelos\/([^/]+)\//.exec(e.path))
    .filter(Boolean)
    .map((m) => m[1]))].sort();
  // `esc-` é do modelo do escritório: pacote que publicasse `esc-*` cairia em cima do modelo que o
  // escritório salvou (o sync só toca o que o pacote declara), e a área-base publica sem prefixo.
  const reservados = ids.filter((id) => id === 'esc' || id.startsWith('esc-'));
  if (!ids.length) return { ids: [], fora: [], semDeclaracao: false, reservados: [] };
  if (prefixo === null) return { ids, fora: [], semDeclaracao: true, reservados };
  if (prefixo === '') return { ids, fora: [], semDeclaracao: false, reservados };
  return { ids, fora: ids.filter((id) => id !== prefixo && !id.startsWith(`${prefixo}-`)), semDeclaracao: false, reservados };
}
