...
import { metaLimiarDoSquadYaml, rubricaDeclaraLimiarEmTexto } from './meta-consenso.js';
import { papelDoStep } from './squad-compile.js';
 ...
 */
export function escalarDeChave(texto, chave) {
...
/** Do `delivery_type` do squad.yaml para os `delivery_type` de skill que servem àquela entrega. */
const TIPOS_DE_SKILL_POR_ENTREGA = {
  'legal-analysis': ['legal-analysis'],
  content: ['public-content'],
  research: ['legal-analysis', 'evidence-report'],
};

/** Devolve os ids das skills declaradas que são de peça (ver `DELIVERY_TYPE_DE_PECA`); `tipos` sai preenchido com o delivery_type de cada skill lida. */
function checarSkillsDeclaradas(dir, skillsDir, issues, steps = [], tipos = new Map()) {
  ...
  for (const id of [...declaradas].sort()) {
    ...
    const textoDaSkill = readFileSync(skillPath, 'utf8');
    const metadata = parseSkillMetadata(textoDaSkill, { fallbackName: id });
    if (metadata?.deliveryType) tipos.set(id, metadata.deliveryType);
    ...
    // Skill sem o contrato de qualidade: o resolvedor do run a bloqueia (`quality-legacy-blocked`).
    // A skill criada no escritório nasce assim, e a criação do squad dizia "estrutura íntegra" até o
    // run parar (revisão v3 do modelo do escritório, M8, 25/09/2026). A camada local vence.
    if (politica.productionEligible) {
      const localPath = join(skillsDir, id, 'SKILL.local.md');
      const efetivo = existsSync(localPath) ? readFileSync(localPath, 'utf8') : textoDaSkill;
      const metaEfetiva = efetivo === textoDaSkill ? metadata : parseSkillMetadata(efetivo, { fallbackName: id });
      if (String(metaEfetiva?.qualityStatus || 'legacy').toLowerCase() === 'legacy' || !efetivo.includes('LEGALSQUAD:HP-CONTRACT:START')) {
        issues.push(issue('warn', 'skill-sem-contrato', `skill "${id}" não tem o contrato de qualidade: o run a bloqueia; rode \`npx legalsquad contract-skills\` na pasta do projeto`));
      }
    }
  }
  return dePeca;
...
/**
 * Skill citada no texto de um agente ou de um passo ("carregar a skill `tabela-da-casa`") que não
 * existe em `skills/`: o agente é mandado carregar o que não há, e nada avisava (revisão v3 do
 * modelo do escritório, M5).
 */
function checarSkillsCitadas(dir, skillsDir, issues) {
  if (!existsSync(skillsDir)) return;
  let passos = [];
  try { passos = readdirSync(join(dir, 'pipeline', 'steps')).filter((f) => f.endsWith('.md')).map((f) => join(dir, 'pipeline', 'steps', f)); } catch { /* sem passos */ }
  const nativas = new Set(NATIVE_RUNTIME_SKILLS);
  const vistas = new Set();
  for (const arquivo of [...arquivosDeAgente(dir), ...passos]) {
    let texto;
    try { texto = readFileSync(arquivo, 'utf8'); } catch { continue; }
    for (const m of texto.matchAll(/\bskills?\s+`([a-z0-9][a-z0-9-]{2,80})`/gi)) {
      const id = m[1];
      if (vistas.has(id) || nativas.has(id)) continue;
      vistas.add(id);
      if (existsSync(join(skillsDir, id, 'SKILL.md')) || existsSync(join(skillsDir, id, 'SKILL.local.md'))) continue;
      issues.push(issue('warn', 'skill-citada-inexistente', `${basename(arquivo)} manda usar a skill "${id}", que não existe em ${skillsDir}`));
    }
  }
}
 ...
 * quem já a fez não pode ver o CI virar vermelho por isso. E a escolha conta
 * quando está ESCRITA: `meta_verifiers: 1` declarado cala o aviso tanto quanto
 * `: 3`. Só a chave ausente avisa.
 */
// As MESMAS réguas de `templates/scripts/run-metricas.mjs` (ehArtefatoDeEntrega):
// rascunho pelo nome e interno pelo nome ficam fora da entrega. Duplicadas aqui
// porque `src/` não importa de `templates/` (o script viaja para o projeto do
// aluno, o motor não); tests/squad-check.test.js prende a igualdade.
export const NOME_DE_RASCUNHO = /(?:^|[-_.])(?:minuta|rascunho|draft|intern[oa])(?:[-_.]|$)/i;
export const NOME_INTERNO = /^(?:revis[ãa]o|aprova[çc][ãa]o|checklist|relat[óo]rio|pesquisa|resumo|diagn[óo]stico|fatos|teses|estrat[ée]gia|intake|foco|gate|termo|confer[êe]ncia|linha|mem[óo]ria|notas|plano|mapa|an[áa]lise|contradi[çc][õo]es|pre-?mortem|temas|anexos|proximos-passos|manifesto|avalia[çc][ãa]o|verifica[çc][ãa]o|meta|contraditor|apoio|persuas[ãa]o)(?:[-_.]|$)/i;

/**
 * Squad que entrega peça e não declara NENHUM artefato que o empacotador
 * aceite (todos são `*-minuta.md`, `revisao.md`, `foco.md`…) termina o run com
 * a peça em "minuta" e o `empacotar.mjs` embrulhando outro `.md` no lugar dela
 * (achado num processo real, 15/09/2026: o pacote saiu com `foco-do-caso.md`).
 * O `build.prompt.md` manda a versão aprovada ir em `output/<peça>-final.md`;
 * aqui isso vira exigência do desenho, não conselho.
 */
function checarArtefatoDeEntrega(pipeline, steps, issues) {
  const declarados = [...parseArtefatosDoPipeline(pipeline), ...steps.flatMap((s) => s.artefatos || [])];
  const nomes = declarados.map((a) => String(a).replace(/^output\//, ''));
  const entregas = nomes.filter((n) => !n.includes('/') && /\.md$/i.test(n) && !NOME_DE_RASCUNHO.test(n) && !NOME_INTERNO.test(n) && !n.startsWith('_') && !n.startsWith('.'));
  if (entregas.length) return;
  const minutas = [...new Set(nomes.filter((n) => NOME_DE_RASCUNHO.test(n)))];
  issues.push(issue(
    'error',
    'entrega-sem-artefato-final',
    `o squad entrega peça, mas nenhum artefato declarado passa no filtro do empacotador${minutas.length ? ` (só rascunhos: ${minutas.join(', ')})` : ''}; declare \`output/<peça>-final.md\` no step que grava a versão aprovada (a minuta continua sendo a versão de trabalho)`
  ));
}
...
function checarVotingDaMeta(y, pipeline, sinais, issues) {
  ...
  // O contrato é "declarado", não ">1". Chave escrita com QUALQUER inteiro
  // (0 e 1 inclusive) é escolha feita e não avisa: desde 11/09/2026 a Discovery
  // pergunta ao profissional se quer o red-team, e o Build grava
  // `meta_verifiers: 1` quando a resposta é "não" — avisar aí seria cobrar de
  // quem respondeu a pergunta que ele esqueceu de responder. Só a chave
  // AUSENTE (ou não-inteira, que `inteiroDeChave` devolve como null) avisa.
  const declarados = [inteiroDeChave(y, 'meta_verifiers'), metaVerifiersDoPipeline(pipeline)]
    .filter((n) => n !== null);
  if (declarados.length) return; // declarado — a escolha foi feita, seja qual for

  issues.push(issue(
    'warn',
    'meta-verifiers-sem-voting-em-peca',
    `${sinais.join('; ')}, e meta_verifiers não é declarado (default 1): `
      ...
      + 'conservador (o mesmo do Citation Gate), ou `meta_verifiers: 1` se o custo for a escolha: '
      + 'o que cala este aviso é a escolha ficar escrita'
  ));
}

/**
 * Critério de sucesso que cobra artefato de step POSTERIOR à Verificação da Meta.
 *
 * A meta se ancora no step que declara `meta_verifiers` (a conferência de
 * entrega, nos squads compilados) e julga o que existe naquela hora. O checklist
 * de protocolo ou de distribuição vem depois, na volta da aprovação. Medido em
 * 24/09/2026: a contestação perdeu o C6 ("reconferidos no checklist") e o HC
 * parte do C2 ("diligência antes da distribuição") porque o checklist ainda não
 * existia quando o avaliador leu a peça; ele cumpria tudo. O avaliador novo não
 * pune o que é de step posterior, mas o critério continua medindo o que a meta
 * não vê: o conserto é no desenho (tirar do critério ou gerar antes da meta).
 *
 * Sinais, do mais forte ao mais fraco, só em step de agente depois da âncora
 * (parada humana não grava artefato que a peça cite): o nome do artefato
 * (`checklist-protocolo`), o miolo do id (`step-13-checklist-protocolo`), a
 * palavra que abre o nome do step ("Checklist"), e "antes d[ao] <palavra do nome
 * do step>" ("antes da distribuição"). Palavra solta do nome ("protocolo",
 * "audiência") não conta: aparece em critério bom a toda hora. Warn: o squad
 * roda, só mede o que não devia.
 */
const PALAVRAS_VAZIAS_DE_STEP = new Set(['step', 'passo', 'etapa', 'de', 'da', 'do', 'das', 'dos', 'e', 'a', 'o', 'para', 'com', 'final']);
const normalizarTermo = (texto) => String(texto || '').normalize('NFD').replace(/[̀-ͯ]/g, '').toLowerCase().replace(/[^a-z0-9]+/g, ' ').trim();

function checarCriterioDepoisDaMeta(y, steps, issues) {
  if (y === null) return;
  const ancora = steps.map((s) => Boolean(s.metaVerifiers)).lastIndexOf(true);
  if (ancora < 0) return; // sem âncora, a meta roda no fim: nada vem depois dela
  const posteriores = steps.slice(ancora + 1).filter((s) => s.tipo !== 'checkpoint');
  if (!posteriores.length) return;
  const criterios = listaDeTopo(y, 'success_criteria');
  criterios.forEach((criterio, i) => {
    const texto = ` ${normalizarTermo(criterio)} `;
    for (const step of posteriores) {
      const palavrasDoNome = normalizarTermo(step.name).split(' ').filter((p) => p.length >= 5 && !PALAVRAS_VAZIAS_DE_STEP.has(p));
      const termos = new Set([
        ...(step.artefatos || []).map((a) => normalizarTermo(basename(String(a)).replace(/\.[a-z0-9]+$/i, ''))),
        normalizarTermo(String(step.id).replace(/^step-?\d+-?/i, '')),
        palavrasDoNome[0] || '',
      ].filter((t) => t.length >= 5));
      const achado = [...termos].find((t) => texto.includes(` ${t} `))
        || palavrasDoNome.flatMap((p) => [`antes da ${p}`, `antes do ${p}`]).find((f) => texto.includes(` ${f} `));
      if (!achado) continue;
      issues.push(issue(
        'warn',
        'criterio-depois-da-meta',
        `success_criteria ${i + 1} cita "${achado}", que é do step ${step.id} ("${step.name}"), posterior à Verificação da Meta `
          + `(ancorada em ${steps[ancora].id}): na hora da meta esse artefato ainda não existe, e o critério mede o que o avaliador não pode ver. `
          + 'Tire a exigência do critério (ela fica no step posterior) ou gere o conteúdo antes da meta, no step de conferência'
      ));
      return; // um aviso por critério basta
    }
  });
}

/**
 * A regra de entrega da Verificação da Meta: `meta_limiar` no squad.yaml.
 *
 * O `squad-state meta-consenso` decide o veredito pela regra estruturada e não
 * extrai número de prosa. Medido em 24/09/2026: nos 8 squads da medição, o
 * "Limiar" do `quality-criteria.md` é regra ("ATENDE nos critérios 1, 4 e 6";
 * "no máximo um PARCIAL, que não pode ser o 1 nem o 6"), e um leitor de número
 * devolvia null em 7 e um 85 de outra frase no oitavo. Então: rubrica que fala
 * de limiar em texto sem `meta_limiar` leva warn (o run cai no padrão do motor,
 * nenhum NAO e nota 85, que não é a regra do squad); `meta_limiar` ilegível é
 * error, porque o comando recusa combinar com regra lida pela metade.
 */
function checarLimiarDaMeta(dir, y, issues) {
  if (y === null) return;
  const nCriterios = listaDeTopo(y, 'success_criteria').length;
  const declarada = metaLimiarDoSquadYaml(y, nCriterios);
  if (declarada.presente) {
    if (declarada.erros.length) issues.push(issue('error', 'meta-limiar-invalido', `${declarada.erros.join('; ')}: o \`meta-consenso\` recusa combinar com essa regra`));
    return;
  }
  const rubrica = join(dir, 'pipeline', 'data', 'quality-criteria.md');
  if (!existsSync(rubrica) || !rubricaDeclaraLimiarEmTexto(readFileSync(rubrica, 'utf8'))) return;
  issues.push(issue(
    'warn',
    'limiar-em-texto-sem-meta-limiar',
    'o pipeline/data/quality-criteria.md declara um limiar em texto, e o squad.yaml não tem `meta_limiar`: o veredito da meta '
      + 'cai no padrão do motor (nenhum NAO e nota 85), não na regra do squad. Transcreva a regra em `meta_limiar` '
      + '(nao_max, parcial_max, atende_obrigatorios, parcial_permitidos, nota_min)'
  ));
}
...
// ---------------------------------------------------------------------------
// Gate do runner desenhado como step do squad: o mesmo trabalho pago duas vezes.
//
// Medido num squad real (réplica cível, 17/09/2026): o Arquiteto desenhou um
// step "Citation Gate Final (votação de 3)" com agente próprio e um step "Gate
// de Sobrevivência ao Resumo", quando o runner já roda os dois nos steps de
// redação, revisão e conferência, com cartório de citações e reabertura por
// código. O step a mais despacha os verificadores de novo, sem o cartório, e
// a peça espera. Aviso, não erro: o squad roda; só paga duas vezes.
// ---------------------------------------------------------------------------
const GATE_DO_RUNNER = /citation[\s_-]?gate|gate[\s_-]*(?:de[\s_-]*)?persuas|sobreviv[êe]ncia[\s_-]*ao[\s_-]*resumo|verifica[cç][aã]o[\s_-]*da[\s_-]*meta|reda[cç][aã]o[\s_-]*gate/i;

function checarGatesDoRunnerComoStep(steps, issues) {
  for (const step of steps) {
    const alvo = `${step.id || ''} ${step.name || ''} ${step.agent || ''}`;
    if (!GATE_DO_RUNNER.test(alvo)) continue;
    issues.push(issue(
      'warn',
      'gate-do-runner-como-step',
      `${step.id}${step.name ? ` ("${step.name}")` : ''}: este gate é do runner (roda nos steps de redação, revisão e `
        + 'conferência, com cartório de citações e reabertura por código); como step do squad ele despacha os '
        + 'verificadores de novo, sem o cartório, e o run paga duas vezes. Remova o step (e o agente fino que só o '
        + 'embrulha); o revisor aciona o verificador-citacoes, e o voting vem de citation_verifiers/meta_verifiers no squad.yaml'
    ));
  }
}

// ---------------------------------------------------------------------------
// Redação sem skill: a peça nasce de memória.
//
// Medido em 18/09/2026 num projeto real: os squads desenhados pelo Arquiteto
// tinham 0 a 4 skills por agente, e os do pacote criminal declaravam 5 a 21 no
// squad.yaml e ZERO no agente que redige. O runner injeta o corpo da skill pelo
// frontmatter do AGENTE (Agent Loading, passo 4); a lista do squad.yaml
// alimenta o gate de runtime e o hook, não o contexto de quem escreve. Aviso,
// não erro: o squad roda; só roda sem o conhecimento que o pacote trouxe.
// ---------------------------------------------------------------------------
const PASSO_DE_REDACAO = /reda[cç][aã]o|redator|minuta/i;
const PASSO_DE_CITATION_GATE = /citation[\s_-]?gate/i;

function checarRedacaoSemSkill(dir, steps, sinais, issues, skillsDePeca = [], { tipos = new Map(), deliveryTypeDoSquad = null } = {}) {
  // Squad que declara outro tipo de entrega (parecer é `legal-analysis`): a skill
  // que o redator precisa é a desse tipo, não uma de `legal-draft` que outro
  // agente use. Sem isto, todo parecer com uma skill de peça no squad.yaml
  // levava o aviso pelo redator certo.
  // O vocabulário do squad.yaml (`legal-draft | legal-analysis | content | research`) não é o
  // das skills (`public-content`, `evidence-report`…): a tradução é aqui, senão o squad de
  // conteúdo nunca acha skill da entrega e o parecer sem skill de parecer passa mudo.
  const tiposDaEntrega = deliveryTypeDoSquad && deliveryTypeDoSquad !== DELIVERY_TYPE_DE_PECA
    ? new Set([deliveryTypeDoSquad, ...(TIPOS_DE_SKILL_POR_ENTREGA[deliveryTypeDoSquad] || [])])
    : null;
  const daEntrega = tiposDaEntrega ? [...tipos].filter(([, t]) => tiposDaEntrega.has(t)).map(([id]) => id) : [];
  // As skills que quem redige tem de carregar: as de peça num squad de peça; as da entrega
  // declarada nos outros (parecer, conteúdo, pesquisa).
  // Squad de peça pelos sinais do motor, ou pelo desenho antigo que traz um step
  // de Citation Gate (os squads do pacote criminal não declaram citation_verifiers).
  const entregaPeca = sinais.length > 0 || steps.some((s) => PASSO_DE_CITATION_GATE.test(`${s.id} ${s.name || ''}`));
  if (!entregaPeca) return;
  const alvosDeRevisao = new Set(steps.filter((s) => s.onReject).map((s) => s.onReject));
  // Skill de peça que só o revisor ou o conferente declara (auditar a peça, conferir a
  // forma) é ferramenta de quem confere, não o que falta a quem escreve. Contá-la levava o
  // aviso a mandar o redator carregar a skill do revisor: medido no desenho da onda
  // extrajudicial de 25/09/2026, confissão de dívida e acordo extrajudicial.
  const agentesQueRedigem = new Set();
  const agentesQueNaoRedigem = new Set();
  for (const s of steps) {
    if (!s.agent) continue;
    const r = alvosDeRevisao.has(s.id) || PASSO_DE_REDACAO.test(`${s.id} ${s.name || ''} ${s.agent}`);
    (r ? agentesQueRedigem : agentesQueNaoRedigem).add(s.agent);
  }
  const declaradasPor = (agentes) => new Set([...agentes].flatMap((a) => {
    const f = join(dir, 'agents', `${a}.agent.md`);
    return existsSync(f) ? skillsDeclaradas(readFileSync(f, 'utf8')) : [];
  }));
  const deQuemRedige = declaradasPor(agentesQueRedigem);
  const deQuemConfere = declaradasPor([...agentesQueNaoRedigem].filter((a) => !agentesQueRedigem.has(a)));
  const soDeQuemConfere = (id) => deQuemConfere.has(id) && !deQuemRedige.has(id);
  const relevantes = (tiposDaEntrega ? daEntrega : skillsDePeca).filter((id) => !soDeQuemConfere(id));
  for (const step of steps) {
    if (!step.agent) continue;
    const redige = alvosDeRevisao.has(step.id) || PASSO_DE_REDACAO.test(`${step.id} ${step.name || ''} ${step.agent}`);
    if (!redige) continue;
    const arquivo = join(dir, 'agents', `${step.agent}.agent.md`);
    if (!existsSync(arquivo)) continue;
    const declaradas = skillsDeclaradas(readFileSync(arquivo, 'utf8'));
    // O step de redação pode carregar a skill de peça pelo texto ("carregue a
    // skill X"): é a segunda porta de injeção que o design prevê, e vale.
    const textoDoStep = step.file && existsSync(join(dir, 'pipeline', step.file)) ? readFileSync(join(dir, 'pipeline', step.file), 'utf8') : '';
    const stepCarregaPeca = [...new Set([...skillsDePeca, ...relevantes])].some((id) => new RegExp(`(^|[^A-Za-z0-9_-])${id.replace(/[.*+?^${}()|[\]\\]/g, '\\$&')}([^A-Za-z0-9_-]|$)`).test(textoDoStep));
    if (declaradas.length > 0) {
      // Declara skill, mas nenhuma é a da entrega, nem o step a carrega pelo texto:
      // a peça de fato nasce sem o conhecimento que o pacote trouxe.
      if (relevantes.length > 0 && !stepCarregaPeca && !declaradas.some((id) => skillsDePeca.includes(id) || daEntrega.includes(id))) {
        const rotulo = tiposDaEntrega ? `skill da entrega (${deliveryTypeDoSquad})` : 'skill de peça';
        issues.push(issue(
          'warn',
          'redacao-sem-skill-de-peca',
          `${step.id} (agente "${step.agent}"): o agente declara ${declaradas.join(', ')} e nenhuma é ${rotulo}; as skills da entrega `
            + `do squad (${relevantes.slice(0, 4).join(', ')}${relevantes.length > 4 ? '…' : ''}) estão só no squad.yaml, e o runner injeta `
            + `o corpo pelo frontmatter do AGENTE: quem escreve não as recebe. Declare a ${rotulo} em agents/${step.agent}.agent.md`
        ));
      }
      continue;
    }
    if (stepCarregaPeca) continue;
    issues.push(issue(
      'warn',
      'redacao-sem-skill',
      `${step.id} (agente "${step.agent}"): o step que redige a peça não declara skill nenhuma no frontmatter do agente. `
        + 'O runner injeta o corpo da skill POR AGENTE (a lista do squad.yaml alimenta o gate e o hook, não o contexto de quem '
        + `escreve): a peça nasce de memória e o Redação Gate não tem contrato de saída para cobrar. Declare em agents/${step.agent}.agent.md `
        + 'a skill da peça e as de análise/estratégia da família (2 a 4), pela shortlist do search-skills'
    ));
  }
}
...
const SECOES_DE_AGENTE = [
  ...
  '## Principles',
  // Até o 0.9.55 os dois títulos levavam travessão («Vocabulary — Always Use»): o squad construído
  // antes continua valendo pelo título antigo, e o aviso sugere o de hoje.
  '## Voice Guidance', ['### Vocabulary: Always Use', '### Vocabulary \u2014 Always Use'], ['### Vocabulary: Never Use', '### Vocabulary \u2014 Never Use'],
...
// Uma seção pode ter título legado: `[atual, ...legados]`; qualquer um vale, e o que falta sai pelo atual.
const secoesAusentes = (texto, secoes) => secoes
  .filter((sec) => ![sec].flat().some((t) => temSecao(texto, t)))
  .map((sec) => [sec].flat()[0]);
...
function checarNomesDosAgentes(dir, issues) {
  for (const arquivo of arquivosDeAgente(dir)) {
    ...
    if (!fm) continue;
    // Agente copiado de outro squad leva o `squad:` de lá: o runner e o painel o atribuem ao squad
    // errado, e o code de outro caso (que pode ter o nome do cliente) viaja junto. A primeira versão
    // do modelo do escritório gravava assim, e o check-squad dizia "estrutura íntegra" (revisão de 24/09/2026, D3).
    const doSquad = escalarDeChave(fm, 'squad');
    if (doSquad !== null && doSquad.replace(/^["']|["']$/g, '') !== basename(dir)) {
      issues.push(issue('warn', 'agente-de-outro-squad', `${basename(arquivo)}: squad "${doSquad}" no frontmatter, mas o agente está em squads/${basename(dir)}/ (copiado de outro squad?)`));
    }
...
// ---------------------------------------------------------------------------
// Marcador de compilação que ficou sem prosa.
//
// `npx legalsquad compilar-squad` grava os arquivos mecânicos do squad e deixa,
// onde vai prosa, um marcador `<!-- LEGALSQUAD:PREENCHER id | instrução -->` que
// o Build substitui. Marcador que sobrevive é seção sem conteúdo: o agente
// nasce sem princípio, o step sem exemplo, o caso-ouro sem input. Erro, não
// aviso: o squad "roda", mas roda com o esqueleto no lugar da regra, e nenhum
// outro gate vê isso (as seções existem; só estão vazias).
// ---------------------------------------------------------------------------
const MARCADOR_DE_COMPILACAO = 'LEGALSQUAD:PREENCHER';

function checarMarcadoresDeCompilacao(dir, issues) {
  const pastas = ['agents', 'pipeline', '_evals'].map((p) => join(dir, p)).filter((p) => existsSync(p));
  for (const pasta of pastas) {
    let arquivos;
    try {
      arquivos = readdirSync(pasta, { recursive: true, withFileTypes: true })
        .filter((e) => e.isFile() && /\.md$/i.test(e.name))
        .map((e) => join(e.parentPath ?? e.path, e.name));
    } catch {
      continue; // pasta ilegível: outros checks já acusam o que falta
    }
    for (const arquivo of arquivos.sort()) {
      const texto = readFileSync(arquivo, 'utf8');
      const n = (texto.match(/LEGALSQUAD:PREENCHER/g) || []).length;
      if (!n) continue;
      const ids = [...texto.matchAll(/LEGALSQUAD:PREENCHER\s+([^\s|]+)/g)].map((m) => m[1]);
      issues.push(issue(
        'error',
        'marcador-de-compilacao',
        `${arquivo.slice(dir.length + 1)}: ${n} marcador(es) ${MARCADOR_DE_COMPILACAO} sem prosa (${ids.join(', ')}); o Build preenche cada um com o conteúdo da seção; esqueleto no lugar da regra não é squad pronto`
      ));
    }
  }
}

// Os papéis que o id do step e o do agente decidem sozinhos, sem o nome livre: quando o id diz
// "protocolo" (ou "revisao", "redacao"...) e o compilador, pelo nome ou pelo título, deu outro papel,
// o agente recebeu o texto fixo do papel errado. Medido na onda extrajudicial (26/09/2026): o
// protocolista da licitação ("Checklist de envio no sistema da licitação") compilou como leitor de
// temas da fase zero, e o check-squad não disse nada.
const PAPEIS_DECIDIDOS_PELO_ID = new Set(['protocolo', 'revisao', 'redacao', 'pesquisa', 'conferencia', 'prazo', 'pre-mortem']);
const QUEM_E_CADA_PAPEL = {
  temas: 'leitor de temas da fase zero, read-only', resumo: 'leitor de resumo da fase zero, read-only', prova: 'leitor de prova da fase zero, read-only',
  leitor: 'leitor da fase zero, read-only', 'pre-mortem': 'adversário do pré-mortem', prazo: 'contador de prazo', protocolo: 'protocolista',
  revisao: 'revisor', redacao: 'redator', pesquisa: 'pesquisador', conferencia: 'conferente', generico: 'genérico',
};

/**
 * O papel que o compilador deu a cada step (em `_build/compilacao.json`) contra o que o id do step
 * e o do agente dizem sozinhos. Só acusa quando o id é inequívoco (um dos papéis de
 * `PAPEIS_DECIDIDOS_PELO_ID`) e o papel compilado é outro: aí foi o nome livre que decidiu.
 */
function checarPapelDosSteps(dir, steps, issues) {
  const caminho = join(dir, '_build', 'compilacao.json');
  if (!existsSync(caminho)) return;
  let manifesto;
  try { manifesto = JSON.parse(readFileSync(caminho, 'utf8')); } catch { return; }
  const nomes = new Map(steps.map((s) => [s.id, s.name]));
  for (const s of Array.isArray(manifesto?.steps) ? manifesto.steps : []) {
    if (!s || s.type !== 'agent' || !s.papel || !s.agent) continue;
    const peloId = papelDoStep({ id: s.id, type: s.type, on_reject: s.on_reject || null }, { id: s.agent });
    if (!PAPEIS_DECIDIDOS_PELO_ID.has(peloId) || peloId === s.papel) continue;
    const nome = nomes.get(s.id);
    issues.push(issue('warn', 'papel-incoerente', `${s.id}: o compilador o classificou como "${s.papel}" (${QUEM_E_CADA_PAPEL[s.papel] || s.papel})${nome ? `, pelo nome "${nome}" ou pelo título do agente` : ''}, mas o id do step e o do agente (${s.agent}) dizem "${peloId}" (${QUEM_E_CADA_PAPEL[peloId]}); o agente recebeu o texto fixo do papel errado. Troque o nome do step ou o título do agente no design e recompile`));
  }
}
...
const SUBCOMANDOS_DA_CLI = Object.freeze([
  ...
  'chefe', 'compilar-squad', 'contract-skills', 'contribuir', 'dashboard', 'detail-skill', 'diagnostico', 'perfil', 'eval-init', 'indexar-skills', 'init', 'install',
  'install-global', 'memoria', 'resolve-skills', 'runs', 'search-acervo', 'search-skills',
  'skills', 'squad-modelo', 'squads', 'uninstall', 'update',
]);
...
function checarContratoDePeca(dir, steps, issues) {
  ...
  for (const rev of revisores) {
    ...
    // Sem gravidade nos fixes, todo fix conta como `alta` (fail-closed no
    // cartório) e o loop volta a gastar rodada inteira em hífen e grafia — o
    // padrão medido num run real: três REJECTs seguidos com a peça certa no
    // mérito. Aviso, não erro: o run funciona; o que se perde é a convergência.
    const doAgenteRevisor = rev.agent
      ? (textos.find((t) => t.onde === `agents/${rev.agent}.agent.md`)?.texto || '')
      : '';
    if (!/gravidade/i.test(`${texto}\n${doAgenteRevisor}`)) {
      issues.push(issue('warn', 'revisao-sem-gravidade', `${rev.id}: nem o step nem o agente revisor instruem a gravidade dos fixes (\`critica\`/\`alta\` reprovam; \`media\`/\`baixa\` vão em \`ajustes\`); sem ela todo fix conta como alto e o loop gasta rodada inteira em forma`));
    }
...
function parseSteps(pipeline) {
  ...
  return ids.map((id) => {
    ...
    return {
      id,
      name: (bloco.match(/^ {4}name: (.+?)\s*$/m)?.[1] || '').replace(/^"(.*)"$/, '$1'),
      ...
      // Onde a Verificação da Meta se ancora (o step que declara o voting dela).
      metaVerifiers: Number(bloco.match(/^[ \t]+meta_verifiers:[ \t]*["']?(\d+)/m)?.[1] ?? NaN) || null,
 ...
 * qualidade roda depois do Redação Gate: `juiz` (peça), `autoridade` (peça a
 * decisor fora do Judiciário: órgão, comissão, tabelião, registrador) e `cliente`
 * (parecer) pagam a sobrevivência ao resumo (4.6); `contraparte` (contrato) paga a
 * consistência interna (4.7, verifica-contrato); `publico` (conteúdo de autoridade,
 * o leitor fixo de `delivery_type: content`) paga citação e redação, sem persuasão:
 * o conteúdo abre com gancho, não com síntese para juiz. Sem reader, é juiz; em
 * squad de conteúdo, é publico.
 */
export const READERS = ['juiz', 'autoridade', 'contraparte', 'cliente', 'publico'];

/** `processo:` do squad.yaml (onda extrajudicial de 25/09/2026): ausente, judicial. */
export const PROCESSOS = ['judicial', 'administrativo', 'nenhum'];
...
export function gatesPorTipo({ entregaPeca, reader } = {}) {
  // Conteúdo não é peça (nenhuma skill legal-draft), mas redige de skill e cita:
  // Redação Gate e Citation Gate rodam; persuasão e contrato, não.
  if (reader === 'publico') return ['citacao', 'redacao', 'meta', 'veto'];
...
export function checkSquad(squad, options = {}) {
  ...
  if (!existsSync(scores)) {
    issues.push(issue('error', 'evals-scores-ausente', '_evals/scores.md não existe: sem log não há regressão a detectar. Semeie com `npx legalsquad eval-init ' + basename(dir) + '`'));
  ...
  if (casos.length === 0) {
    issues.push(issue('error', 'caso-ouro-ausente', '_evals/casos/ sem nenhum caso: a avaliação não é repetível. Semeie o esqueleto com `npx legalsquad eval-init ' + basename(dir) + '` (a rubrica sai do squad.yaml; o caso fictício você escreve)'));
  }
  ...
  checarMarcadoresDeCompilacao(dir, issues);
  ...
  checarPapelDosSteps(dir, steps, issues);
  checarAgentesNoSquadYaml(y, steps, issues);
  // `raizDoProjeto`: quem compila numa pasta temporária (a conferência do modelo do escritório antes
  // de gravar) aponta o projeto de verdade, onde moram os especialistas.
  checarReusoDeEspecialistas(dir, options.raizDoProjeto ? join(options.raizDoProjeto, 'squads') : squadsDir, steps, issues, options.homeDir ?? homedir());
  ...
  const tiposDeSkill = new Map();
  const skillsDePeca = checarSkillsDeclaradas(dir, skillsDir, issues, steps, tiposDeSkill);
  checarSkillsCitadas(dir, skillsDir, issues);
  ...
  const deliveryTypeDoSquad = y === null ? null : escalarDeChave(y, 'delivery_type');
  const conteudo = deliveryTypeDoSquad === 'content';
  if (readerDeclarado && !READERS.includes(readerDeclarado)) {
    issues.push(issue('warn', 'reader-desconhecido', `reader: "${readerDeclarado}": use juiz, autoridade, contraparte, cliente ou publico (sem reader, é juiz; em squad de conteúdo, publico)`));
  } else if (conteudo && readerDeclarado && readerDeclarado !== 'publico') {
    // O compilador normaliza; um squad.yaml escrito à mão fica com a prosa da ponta errada.
    issues.push(issue('warn', 'reader-incoerente', `reader: "${readerDeclarado}" em squad de conteúdo (delivery_type: content): o leitor do conteúdo é publico; com outro reader o redator recebe a prosa de peça ou parecer`));
  } else if (readerDeclarado === 'publico' && deliveryTypeDoSquad && !conteudo) {
    issues.push(issue('warn', 'reader-incoerente', `reader: publico com delivery_type: ${deliveryTypeDoSquad}: publico é o leitor de conteúdo (delivery_type: content)`));
  }
  const processoDeclarado = y === null ? null : escalarDeChave(y, 'processo');
  if (processoDeclarado && !PROCESSOS.includes(processoDeclarado)) {
    issues.push(issue('warn', 'processo-desconhecido', `processo: "${processoDeclarado}": use judicial, administrativo ou nenhum (sem a chave, é judicial); o Redação Gate e o empacotador leem daqui se há autos e folha`));
  }
  reader = READERS.includes(readerDeclarado) ? readerDeclarado : conteudo ? 'publico' : 'juiz';
  ...
  // --- Verificação da Meta: critério que cobra artefato de step posterior a ela ---
  checarCriterioDepoisDaMeta(y, steps, issues);

  // --- Verificação da Meta: a regra de entrega é estruturada, não prosa ---
  checarLimiarDaMeta(dir, y, issues);

  // --- Redação Gate: o REJECT precisa de rota de volta declarada ---
  checarOnRejectDeRedacao(sinais, steps, issues);

  // --- Gate do runner como step: custo em dobro, avisado ---
  checarGatesDoRunnerComoStep(steps, issues);

  // --- Redação sem skill: a peça nasce de memória ---
  checarRedacaoSemSkill(dir, steps, sinais, issues, skillsDePeca, { tipos: tiposDeSkill, deliveryTypeDoSquad });

  // --- Gate 4 do Arquiteto (parte mecânica): só para quem entrega peça ---
  if (sinais.length > 0) checarContratoDePeca(dir, steps, issues);

  // --- Quem entrega peça precisa de um artefato que o empacotador ACEITE ---
  if (sinais.length > 0) checarArtefatoDeEntrega(pipeline, steps, issues);
