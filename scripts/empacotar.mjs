 ...
 *   PROXIMOS-PASSOS.md       prazo TRANSCRITO do `prazo-fatal.json` do motor determinístico e dos
 *                            checkpoints (nunca calculado aqui); protocolo após revisão
...
export function cruzarAnexos(documentos, textoPeca) {
  ...
  for (const doc of documentos) {
    ...
    if (formas.some((f) => texto.includes(f))) casou = 'arquivo';
      ...
      // plural de "-ão" é "-ões" ("declarações à defesa", "certidões"): sem isso o documento saía como "não citado"
      const formasDoTipo = tipo.endsWith('ao') ? `${escapeRe(tipo.slice(0, -2))}(?:ao|oes)` : `${escapeRe(tipo)}s?`;
      if (new RegExp(`(?<![a-z0-9])${formasDoTipo}(?![a-z0-9])`).test(texto)) casou = 'tipo';
    }
...
export function gerarTermo(t) {
  ...
  if (reab.length) {
    ...
    L.push('');
    L.push('| # | Reaberto em | Modo | Entrega anterior | Pedido (dado sigiloso mascarado) |');
    L.push('|---|---|---|---|---|');
    for (const r of reab) L.push(`| ${celula(r.numero)} | ${celula(r.em) || 'carimbo ausente'} | ${celula(r.modo)} | ${celula(r.versao_anterior) || '?'}${r.entregue_em ? ` (${celula(r.entregue_em)})` : ''} | ${celula(truncar(mascararSigilo(r.pedido || ''), 240)) || '(vazio)'} |`);
    L.push('');
  }
...
export function localizarPrazoFatal({ outputDir, artefatoPath }) {
  ...
  while (pasta.startsWith(raizOutput)) {
    candidatos.push(join(pasta, PRAZO_FATAL_ARQUIVO));
    let subpastas;
    try { subpastas = readdirSync(pasta, { withFileTypes: true }).filter((e) => e.isDirectory() && /^v\d+$/.test(e.name)).map((e) => e.name); } catch { subpastas = []; }
    subpastas.sort((a, b) => Number(b.slice(1)) - Number(a.slice(1)));
    for (const v of subpastas) candidatos.push(join(pasta, v, PRAZO_FATAL_ARQUIVO));
    if (pasta === raizOutput) break;
    pasta = dirname(pasta);
  }
  candidatos.push(join(raizOutput, 'diagnostico', PRAZO_FATAL_ARQUIVO));
  const arquivo = [...new Set(candidatos)].find((c) => existsSync(c));
...
export function gerarProximosPassos(p) {
  ...
  if (!comPrazo.length) {
    if (!pf) L.push(`- Prazo: ${PRAZO_NAO_INFORMADO} — nenhum \`${PRAZO_FATAL_ARQUIVO}\` no run e nenhum checkpoint do \`run-state.json\` menciona prazo ou data fatal. O empacotador **não calcula** prazo.`);
  } else {
    L.push(pf ? 'O que os checkpoints registraram sobre prazo (transcrito; confira contra a data-limite acima):' : 'Transcrito dos checkpoints (o empacotador **não calcula** prazo — confira a contagem):');
    L.push('');
...
export async function empacotar({ squadDir, runId = null, artefato = null, semPdf = false, agora = null, env = process.env } = {}) {
  ...
  writeFileSync(join(pacoteDir, 'PROXIMOS-PASSOS.md'), gerarProximosPassos({
    squad: squadNome, runId: runIdFinal, squadRel, run, pendencias, manifesto, citacoesProblema, anexos, pdf, docx: nomeDocx, prazoFatal, geradoEm,
  }));
