...
def extract_scene_candidates(
    ...
    cmd += [
        "-i", str(Path(video_path).resolve()),
        "-vf", vf,
        "-fps_mode", "vfr",  # -vsync foi removido no ffmpeg 8; -fps_mode existe desde o 5.1
    ]
...
def extract_uniform(
    ...
    ``dedup`` (padrao True) colapsa apenas quadros VISUALMENTE IDENTICOS (limiar
    conservador) — para cortar custo em cenas estaticas sem perder nenhuma
    mudanca; ``dedup=False`` mantem literalmente todos os quadros da cadencia.
