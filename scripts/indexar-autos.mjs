...
/** Páginas (1-based) abaixo do mínimo de caracteres: sem camada de texto, provavelmente digitalizadas. */
export function paginasSemTexto(paginas) {
  const out = [];
  paginas.forEach((p, i) => { if (p.replace(/\s+/g, '').length < MIN_CARACTERES_PAGINA) out.push(i + 1); });
  return out;
}

/**
 * Cobertura depois da conversão (`autos-para-md.py` grava `_md/<slug>/_manifesto.json`
 * com a origem de cada folha: nativo, ocr ou vazia): a página que ficou `vazia`
 * mesmo depois do OCR é a que ninguém leu. Sem manifesto, vale o pdftotext.
 */
function paginasSemTextoDepoisDoOcr(dir, markdownRel) {
  ...
  try {
    ...
    return folhas.filter((f) => f && f.origem === 'vazia').map((f) => Number(f.pagina)).filter(Number.isFinite);
  } catch {
    return null;
  }
}
...
if (process.argv[1] && resolve(process.argv[1]) === fileURLToPath(import.meta.url)) {
  ...
  try {
    ...
    const cob = cobertura(indice);
    if (cob.paginas > 0) {
      const detalhe = cob.semTexto ? ` — ${cob.semTexto} sem texto (${cob.faixas.join('; ')}): rode \`npm run autos:md\` (OCR) e reindexe antes do primeiro step` : ' — leitura integral';
