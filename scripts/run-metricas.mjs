...
export const PENDING_MARKER = /\[(?:N[ÃA]O[ _]VERIFICAD[OA]|DIVERGENTE|CONFERIR|A[ _]CONFERIR|CONFIRMAR|A[ _]CONFIRMAR|VERIFICAR|HIP[ÓO]TESE|CITA[ÇC][ÃA]O[ _]PENDENTE|FONTE[ _]PENDENTE|PENDENTE[ _]DE[ _]VERIFICA[ÇC][ÃA]O)(?:(?:\s+|\s*[:—–-])[^\]]*)?\]/gi;
/** Tema sem âncora apontado pelo verificador de persuasão — contado à parte (não trava hoje). */
export const TEMA_MARKER = /\[TEMA[ _]A[ _]CONFERIR(?:(?:\s+|\s*[:—–-])[^\]]*)?\]/gi;
...
const INTERNAL_NAME = /^(?:revis[ãa]o|aprova[çc][ãa]o|checklist|relat[óo]rio|pesquisa|resumo|diagn[óo]stico|fatos|teses|estrat[ée]gia|intake|foco|gate|termo|confer[êe]ncia|linha|mem[óo]ria|notas|plano|mapa|an[áa]lise|contradi[çc][õo]es|pre-?mortem|temas|anexos|proximos-passos|manifesto)(?:[-_.]|$)/i;
