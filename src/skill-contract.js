...
import { extractFrontMatter, parseList, parseScalar, parseSkillMetadata } from './frontmatter.js';
import { discoverSkillCatalog } from './skill-catalog.js';
import { ehDoDeposito } from './deposito.js';
import {
...
/** O texto sem a linha `quality_profile_source`, para comparar um carimbo novo com um antigo. */
function semALinhaDeFonte(texto) {
  return texto.replace(/^[ \t]*quality_profile_source:[^\n]*\n/m, '');
}

function normalizeFrontmatter(raw, entry, profileId, profile, evalIds, contractVersion, qualityStatus, profileSource = 'inferido') {
  ...
  for (const block of top) {
    if (['name', 'description', 'license', 'compatibility', 'allowed-tools', 'metadata'].includes(block.key)) continue;
  ...
  const updates = [
    ...
    simpleBlock('quality_profile_source', profileSource),
...
export function contractSkillCatalog({ root, force = false, dryRun = false, profilesPath } = {}) {
  ...
  const changed = [];
  const skipped = [];
  // Skill do curador (veio do depósito da máquina, `deposito.js`): o contrato é
  // dele, não deste projeto. Fica de fora antes de qualquer trabalho, por
  // checagem POSITIVA de posse (mesmo inode ou mesmo conteúdo do depósito), e
  // não pela falha da escrita, que não acontece como root nem em modo cópia.
  // Antes, este passo rebaixava em silêncio o contrato v6 de uma área inteira
  // para v5 (medido na área empresarial).
  const protegidas = [];
  const invalidDeclarations = [];
  const revertedEdits = [];

  for (const entry of catalog.entries) {
    // A posse é por ARQUIVO: com `SKILL.local.md` do usuário, o `skillPath` é
    // dele, mas `references/high-performance-contract.md` pode ser o link do
    // curador. Qualquer alvo do depósito deixa a skill inteira de fora.
    const referenciaDoCurador = join(dirname(entry.skillPath), 'references', 'high-performance-contract.md');
    if (ehDoDeposito(entry.skillPath) || ehDoDeposito(referenciaDoCurador)) {
      protegidas.push(entry.id);
      continue;
    }
    // Perfil DECLARADO no frontmatter vence a inferência por padrão de id: é a porta
    // que `skill-quality.js` documenta ("a skill declara metadata.quality_profile e
    // essa declaração tem precedência") e o avaliador já honra. Aqui o contrato a
    // ignorava e reescrevia o perfil pelo id: uma skill de conteúdo chamada
    // "ganchos" nascia `legal-analysis` mesmo declarando `authority-content`.
    //
    // A proveniência fica gravada (`quality_profile_source: declarado | inferido`),
    // porque o carimbo apagava a distinção: na segunda passada, o perfil que o
    // motor inferiu lia-se como declarado pelo curador, e uma declaração inválida
    // sumia sem aviso (revisão de 20/09/2026, C24/V6). Regras, na ordem:
    // - declaração inválida cai na inferência e sai no relatório;
    // - carimbo antigo (tem `contract_version`, não tem linha de fonte): o valor
    //   só conta como declarado quando DIFERE do que o classificador inferiria,
    //   que é a única evidência de escolha humana; igual à inferência é `inferido`.
    //   Sem isso, a primeira passada deste motor carimbava `declarado` em todo o
    //   corpo já contratado (medido: 11 de 11 na fixture) e congelava o defeito;
    // - com a fonte anterior `inferido`, o classificador decide de novo; se o
    //   curador editou o perfil e esqueceu a linha de fonte, o valor é revertido
    //   COM aviso (`reverted_edits`), nunca em silêncio;
    // - para declarar, o curador escreve o perfil e a linha `quality_profile_source: declarado`.
    const declarado = entry.frontmatter ? parseScalar(entry.frontmatter, 'quality_profile') : null;
    const fonteAnterior = entry.frontmatter ? parseScalar(entry.frontmatter, 'quality_profile_source') : null;
    const carimboAntigo = fonteAnterior === null && Boolean(entry.frontmatter && parseScalar(entry.frontmatter, 'contract_version'));
    const inferido = classifySkillQualityProfile(entry);
    const declaracaoValida = Boolean(declarado && profiles.profiles[declarado]);
    if (declarado && !declaracaoValida) invalidDeclarations.push({ id: entry.id, declarado, inferido });
    let profileSource = 'inferido';
    if (declaracaoValida) {
      if (fonteAnterior === 'declarado') profileSource = 'declarado';
      else if (fonteAnterior === 'inferido') { if (declarado !== inferido) revertedEdits.push({ id: entry.id, editado: declarado, inferido }); }
      else if (carimboAntigo) profileSource = declarado !== inferido ? 'declarado' : 'inferido';
      else profileSource = 'declarado';
    }
    const profileId = profileSource === 'declarado' ? declarado : inferido;
    ...
    // Carimbo antigo cuja única diferença é a linha de fonte nova: reescrever mudaria
    // os bytes de todo o corpo já contratado numa passada só, e é o SHA desses bytes
    // que amarra a evidência de promoção (CLAUDE.md, SPEC §6.8). Sem `--force`, a
    // linha entra quando a skill mudar por outro motivo; a proveniência inferida
    // pela ausência da linha vale igual (`carimboAntigo` acima).
    if (!force && carimboAntigo && existsSync(referencePath) && semALinhaDeFonte(migrated) === entry.raw) {
      skipped.push(entry.id);
      continue;
    }
  ...
  // Segunda rede: um arquivo sem bit de escrita (link do depósito que a posse
  // não reconheceu, ou mount somente leitura) não derruba o comando; fica
  // contado como "sem permissão", que é o que de fato aconteceu.
  const semPermissao = [];
  if (!dryRun) {
  ...
  return {
    ...
    changed: changed.length - semPermissao.length,
    skipped: skipped.length,
    protected: protegidas.length,
    protected_ids: protegidas,
    no_permission: semPermissao.length,
    no_permission_ids: semPermissao,
...
export function generateSkillOpenAiMetadata({ root, dryRun = false, profilesPath } = {}) {
  ...
  let generated = 0;
  for (const entry of catalog.entries) {
    // `agents/openai.yaml` do curador fica como veio (posse conferida no alvo).
    if (ehDoDeposito(entry.skillPath) || ehDoDeposito(join(dirname(entry.skillPath), 'agents', 'openai.yaml'))) continue;
    ...
    if (!dryRun) {
      ...
      try {
        ...
        // Sem bit de escrita (link do depósito, mount somente leitura): fica como está.
        if (erro?.code !== 'EACCES' && erro?.code !== 'EPERM') throw erro;
        continue;
      }
    }
