...
function blockedDecision(id, reasons, record = null) {
  return {
    ...
    // Supervisão por run não existe mais: a supervisão é a revisão humana da
    // peça, que todo run já exige. O campo fica por compatibilidade de shape.
...
function evaluateBasePolicy(record) {
  const reasons = [];
  ...
  // `contracted` executa em produção (decisão do dono, 18/09/2026): é o estado
  // de 7.455 das 7.457 skills de um catálogo real, e exigir um "sim" por run
  // para cada uma era ruído que ensinava o Arquiteto a ligar menos skills. A
  // validação real são os gates (Redação, Citation, Meta) e a revisão humana
  // da peça, que nenhum ritmo dispensa. Preview, deprecated, quarantined,
  // legacy, hard fail estrutural e promoção sem evidência continuam bloqueados.
...
export function evaluateSkillRuntime(record, options = {}) {
  ...
  // `options.supervised` segue aceito (squads e prompts antigos passam a flag) e
  // não muda nada: não há mais estado que dependa dela.
  const reasons = evaluateBasePolicy(record);
  let fallback = null;
  ...
  if (String(record.lifecycle || '').toLowerCase() === 'pilot') {
    ...
    if (!fallbackId) {
      ...
      if (!fallbackRecord) {
        ...
        const fallbackReasons = evaluateBasePolicy(fallbackRecord);
  ...
  return {
    id: record.id,
    allowed: true,
    disposition: highPerformanceEligible ? 'high-performance' : 'contracted',
...
export function resolveSkillRuntime(skillIds, options = {}) {
  const mode = ['selection', 'explicit-selection'].includes(options.mode)
    ? options.mode
    : 'execution';
  // Um argumento só pode trazer vários ids ("a b c" entre aspas, ou "a,b,c"):
  // o chefe monta a lista numa string e o shell a entrega inteira. Sem separar,
  // o id composto não existe no catálogo e o gate bloqueia skills instaladas.
