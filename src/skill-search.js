...
import { arquivosDoDeposito, ligacaoDoProjeto, packLigavel, slugDoPack } from './deposito.js';
...
export function skillsForaDasAreas(rootDir, query, limite = 5) {
  ...
  for (const [path, info] of registros) {
    ...
    if (!m || packLigavel(info.pack_id, ligacao.areas)) continue;
...
export function searchSkillCatalog(query, rootDir, options = {}) {
  ...
  const ranked = candidatos
    .map((match) => {
      ...
      const bonusDeArea = casaArea(grupo, areaTokens) ? BONUS_DE_AREA : 0;
      return {
        entry,
        quality,
        match,
    ...
    .map(({ entry, quality, match, rank, substancia: sub, grupo }) => ({
      ...
      grupo,
      area_casou: casaArea(grupo, areaTokens),
...
export function skillSearchCli(query, targetDir, values = {}) {
  ...
  if (result.nao_ligadas?.length) {
    ...
    for (const s of result.nao_ligadas) console.log(`  - ${s.id} — área ${s.area}; para ligar: \`npx legalsquad acervo areas <áreas atuais> ${s.area}\` (sem rede)`);
  }
  return result;
}
