...
export const OFFICIAL_TOP_LEVEL_KEYS = ['name', 'description', 'license', 'compatibility', 'allowed-tools', 'metadata'];
...
export function evaluateSkillQuality(entry, {
  ...
  const compatibility = parseScalar(entry.frontmatter, 'compatibility');
  const compatibilityValida = compatibility === null || compatibility === undefined || String(compatibility).length <= 500;
  // Os campos de topo do padrão aberto Agent Skills: name, description e os opcionais
  // license, compatibility (requisitos de ambiente, até 500 caracteres), metadata e
  // allowed-tools. `compatibility` faltava aqui e 66 skills de uma área, escritas no
  // padrão, reprovavam por "frontmatter não oficial" (16/09/2026).
  const unexpectedTopLevel = topLevelKeys.filter(
    (key) => !OFFICIAL_TOP_LEVEL_KEYS.includes(key),
  );
  ...
  const checks = {
    official_frontmatter: unexpectedTopLevel.length === 0 && compatibilityValida,
  ...
  const hardFails = [];
  if (!checks.official_frontmatter) hardFails.push(`frontmatter não oficial: ${[...unexpectedTopLevel, ...(compatibilityValida ? [] : ['compatibility com mais de 500 caracteres'])].join(', ')}`);
...
/**
 * As skills que o resolvedor de runtime recusaria por defeito ESTRUTURAL, entre
 * as que se declaram de produção (`active`/`pilot`): hard fail ou piso de 90 não
 * atingido (o mesmo teste de `structural-gate-failed` em
 * src/skill-runtime-policy.js). `preview`, `deprecated` e `quarantined` ficam
 * de fora de propósito: o resolvedor as recusa também, mas por decisão
 * declarada do curador, não por defeito que subiu calado. É o que o `build-area`
 * e o `diagnostico` conferem.
 */
export function skillsRecusadasEmProducao(results) {
  return (Array.isArray(results) ? results : [])
    .filter((r) => ['active', 'pilot'].includes(String(r?.lifecycle || '').toLowerCase()))
    .filter((r) => (Array.isArray(r.hardFails) && r.hardFails.length > 0) || Number(r.score || 0) < 90)
    .map((r) => ({
      id: r.id,
      lifecycle: r.lifecycle,
      score: r.score,
      motivos: Array.isArray(r.hardFails) && r.hardFails.length ? r.hardFails : [`piso estrutural não atingido (${r.score} < 90)`],
    }));
}
