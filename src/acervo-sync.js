...
export async function executarSync(plano, ambiente, estadoAnterior = { packs: {} }) {
  ...
  // Um pacote que precisa de conteúdo NÃO é baixado também "só pelo catálogo":
  // o conteúdo já traz o catálogo (uma aplicação, um registro, um índice).
  // Hoje as duas URLs apontam para o mesmo pacote inteiro; sem esta poda, cada
  // atualização baixava, aplicava e indexava o pacote duas vezes.
  const comConteudo = new Set((plano.baixarConteudo || []).map((p) => p.pack_id));
  const alvos = [
    ...(plano.baixarCatalogo || []).filter((p) => !comConteudo.has(p.pack_id)).map((pack) => ({ pack, url: pack.catalog?.url || pack.content?.url })),
    ...(plano.baixarConteudo || []).map((pack) => ({ pack, url: pack.content?.url || pack.catalog?.url })),
  ];

  for (const { pack, url } of alvos) {
    try {
