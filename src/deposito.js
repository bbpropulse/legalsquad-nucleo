 ...
 */
export function registrarPack(deposito, packId, versao, arquivos) {
  exigirPackId(packId);
  ...
  const outros = removidos.length ? caminhosDeOutrosPacks(deposito, packId) : new Set();
  const soDeste = removidos.filter((a) => !outros.has(a.path));
  ...
  invalidarCache();
  return { removidos: soDeste.length };
}
 ...
 * classificar).
 */
export function packLigavel(packId, areas) {
  ...
  return areas.some((a) => {
    ...
    return escolhido === slug || slug.endsWith(`-${escolhido}`) || slug === `direito-${escolhido}` || slug === `direito-do-${escolhido}` || slug === `direito-da-${escolhido}`;
  });
}
...
export function ligarDeposito(projeto, { deposito = raizDoDeposito(), link = linkSync, forcar = false, estado = null, areas = null } = {}) {
  ...
  for (const [path, info] of todosDeclarados) {
    if (info.pack_id) packsVistos.add(info.pack_id);
    if (packLigavel(info.pack_id, areas)) declarados.set(path, info);
    else foraDasAreas.set(path, info);
  }
  resumo.packs = packsVistos.size;
  resumo.packsLigados = [...packsVistos].filter((id) => packLigavel(id, areas)).length;
    ...
    try {
      ...
      if (sa) {
        ...
        if (!identico && !intocadoDesdeALigacao) {
          ...
          resumo.preservados++;
          avisar(`${path}: diferia do pacote; a sua versão ficou em ${basename(bak)}`);
        }
