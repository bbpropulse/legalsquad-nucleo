...
export function contractSkillsProject(targetDir, { force = false } = {}) {
  ...
  console.log(
    ...
    + (contract.protected ? `, ${contract.protected} do depósito da máquina (contrato do curador, intocadas)` : '')
    + (contract.no_permission ? `, ${contract.no_permission} sem permissão de escrita (não alteradas)` : '')
    + '; '
  ...
  // Declaração que o motor não reconhece não pode sumir em silêncio: o carimbo grava o
  // perfil inferido (com `quality_profile_source: inferido`) e o curador fica sabendo.
  for (const d of contract.invalid_declarations || []) {
    console.warn(`  ⚠ ${d.id}: quality_profile «${d.declarado}» não existe em skill-quality-profiles.json; contratada como «${d.inferido}» (quality_profile_source: inferido). Corrija a declaração para um perfil existente.`);
  }
  for (const d of contract.reverted_edits || []) {
    console.warn(`  ⚠ ${d.id}: quality_profile «${d.editado}» com quality_profile_source: inferido; o classificador decidiu «${d.inferido}» e o valor foi revertido. Para declarar «${d.editado}», escreva quality_profile_source: declarado ao lado.`);
  }
  if (!result.ok) {
