...
import { parseBestPracticesCatalog, parseBestPracticesCatalogDir } from './best-practices-catalog.js';
...
export function renderSkillIndex(catalog) {
  ...
  yaml += '  contracted_execution_requires: revisao-humana-da-peca\n';
  ...
  yaml += '  command: "npx legalsquad search-skills --query <capability> --area <area do squad> --limit 12 --json"\n';
  yaml += '  max_prompt_results: 12\n';
...
// Uma instalação tem N áreas e N catálogos (`_catalog.<area>.yaml`), além do
// `_catalog.yaml` legado: o alvo canônico de uma skill pode estar em qualquer
// um. Ler só o nome fixo fazia toda best-practice de pacote "não resolver".
function parseBestPracticeIds(path) {
  const ids = new Set(parseBestPracticesCatalog(path).map((entrada) => entrada.id));
  if (path) for (const entrada of parseBestPracticesCatalogDir(dirname(path))) ids.add(entrada.id);
  return ids;
}
