...
import {
  ...
  packLigavel, slugDoPack, normalizarSlugDeArea, mesmasAreas, PACKS_SEMPRE_LIGADOS,
  registrarPack, removerPack, carimboDoDeposito, indexarPackNoDeposito, packsDeAcervoSemIndice, packIdSeguro, arquivosDoDeposito,
} from './deposito.js';
...
function imprimirEstado(estado, agora, deposito, targetDir, areas = null) {
  ...
  if (ehProjeto(targetDir)) {
    ...
    console.log(`  áreas ligadas: ${descreverAreas(areas, estado)}`);
  }
  if (dias !== null) {
...
export function descreverAreas(areas, estado) {
  const ids = Object.keys(estado?.packs || {});
  if (!areas) return `todas as áreas (${ids.length} pacote(s))`;
  const ligados = ids.filter((id) => packLigavel(id, areas));
...
export async function acervoCli(sub, targetDir, values = {}, agora = Date.now()) {
  ...
  if (sub === 'areas') {
    ...
    if (pedidas.length === 0 && !values.todas) {
      ...
      console.log(`  este projeto liga: ${descreverAreas(areas, estado)}`);
      for (const a of lista) {
        const ligada = packLigavel(a.packs[0], areas);
    ...
    if (!values.todas) {
      ...
      const desconhecidas = pedidas.filter((p) => !disponiveis.some((slug) => packLigavel(`area.${slug}`, [p])));
    ...
    console.log(`  áreas deste projeto: ${descreverAreas(escolhidas, estado)}`);
  ...
  try {
    resultado = await executarSync(plano, {
      ...
      aplicar: (pack, manifesto, entidades) => {
        ...
        registrarPack(deposito, pack.pack_id, pack.latest, arquivos);
