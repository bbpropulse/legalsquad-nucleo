...
export const NOME_INTERNO = /^(?:revis[ãa]o|aprova[çc][ãa]o|checklist|relat[óo]rio|pesquisa|resumo|diagn[óo]stico|fatos|teses|estrat[ée]gia|intake|foco|gate|termo|confer[êe]ncia|linha|mem[óo]ria|notas|plano|mapa|an[áa]lise|contradi[çc][õo]es|pre-?mortem|temas|anexos|proximos-passos|manifesto)(?:[-_.]|$)/i;
...
function checarRedacaoSemSkill(dir, steps, sinais, issues, skillsDePeca = [], { tipos = new Map(), deliveryTypeDoSquad = null } = {}) {
  ...
  const daEntrega = deliveryTypeDoSquad && deliveryTypeDoSquad !== DELIVERY_TYPE_DE_PECA
    ? [...tipos].filter(([, t]) => t === deliveryTypeDoSquad).map(([id]) => id)
    : [];
  ...
  for (const step of steps) {
    ...
    const stepCarregaPeca = skillsDePeca.some((id) => new RegExp(`(^|[^A-Za-z0-9_-])${id.replace(/[.*+?^${}()|[\]\\]/g, '\\$&')}([^A-Za-z0-9_-]|$)`).test(textoDoStep));
    if (declaradas.length > 0) {
      // Declara skill, mas nenhuma é a da peça, nem o step a carrega pelo texto:
      // a peça de fato nasce sem o conhecimento que o pacote trouxe.
      if (skillsDePeca.length > 0 && !stepCarregaPeca && !declaradas.some((id) => skillsDePeca.includes(id) || daEntrega.includes(id))) {
        issues.push(issue(
          'warn',
          'redacao-sem-skill-de-peca',
          `${step.id} (agente "${step.agent}"): o agente declara ${declaradas.join(', ')} e nenhuma é skill de peça; as skills de peça `
            + `do squad (${skillsDePeca.slice(0, 4).join(', ')}${skillsDePeca.length > 4 ? '…' : ''}) estão só no squad.yaml, e o runner injeta `
            + `o corpo pelo frontmatter do AGENTE: quem escreve não as recebe. Declare a skill da peça em agents/${step.agent}.agent.md`
        ));
      }
      continue;
    }
...
const SUBCOMANDOS_DA_CLI = Object.freeze([
  ...
  'chefe', 'compilar-squad', 'contract-skills', 'detail-skill', 'diagnostico', 'perfil', 'eval-init', 'indexar-skills', 'init', 'install',
  'install-global', 'memoria', 'resolve-skills', 'runs', 'search-acervo', 'search-skills',
  'skills', 'squad-modelo', 'uninstall', 'update',
]);
...
export function checkSquad(squad, options = {}) {
  ...
  // --- Redação sem skill: a peça nasce de memória ---
  checarRedacaoSemSkill(dir, steps, sinais, issues, skillsDePeca, { tipos: tiposDeSkill, deliveryTypeDoSquad: y === null ? null : escalarDeChave(y, 'delivery_type') });
