---
name: legalsquad
description: "LegalSquad — Orquestração multi-agente para a prática jurídica. Cria e executa squads: peças, recursos, prazos/intimações, triagem de clientes e conteúdo de autoridade. A área do Direito vem do pacote instalado."
...
hooks:
  ...
  PreToolUse:
    - matcher: "Write|Edit"
      hooks:
        - type: command
          command: 'node "${CLAUDE_PROJECT_DIR}/.claude/hooks/guarda-memoria.mjs"'
          statusMessage: "LegalSquad · guarda de memória (LGPD)"
  PostToolUse:
    - matcher: "Write|Edit"
      hooks:
        - type: command
...
8. **Áreas ligadas nesta pasta.** Com as áreas de atuação fechadas, rode `npx legalsquad acervo areas` (lista o que o depósito desta máquina tem, com ● no que está ligado). Se o escritório atua em poucas áreas, ofereça em uma linha ligar só elas («Ligo nesta pasta só direito civil e consumidor? As outras continuam no depósito da máquina e eu ofereço ligar quando um pedido precisar») e, com o sim, rode `npx legalsquad acervo areas <área> [<área>…]` (sem rede; a busca fica mais precisa e a pasta mais leve). Sem resposta, deixe **todas** (o padrão); `npx legalsquad acervo areas --todas` volta a tudo.
9. Lembre o usuário de popular o `acervo/` (jurisprudência/doutrina/teses) e rodar
   `npm run indexar-acervo` após adicionar material.
10. Show the main menu.
