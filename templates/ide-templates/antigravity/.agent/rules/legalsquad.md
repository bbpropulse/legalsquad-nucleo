...
1. **Decida quem atende** pela ordem REUSAR › ADAPTAR › CRIAR: squad existente em `squads/` **da área do caso** · agente especialista em `.claude/agents/` · tarefa pontual ad-hoc · se for recorrente e nada cobrir, **proponha criar** um squad (só com o "sim" do usuário). Antes de tudo, a **área do caso** (pedido › autos › `Áreas de atuação` do `company.md`) é filtro: `npx legalsquad squads --area <área> --json` separa os squads desta pasta da área (`da_area`) dos de outra área, que nunca entram na shortlist, mesmo com o mesmo nome de peça (os squads prontos vêm dos pacotes: o criminal traz seis, o trabalhista três; um caso cível não reusa nenhum deles); `squad-modelo --para ... --area <área>` faz o mesmo com os modelos. Para skills, gere uma shortlist compacta com `npx legalsquad search-skills --query "<capability sem dados do caso>" --limit 8 --json`, aplique o resolvedor e só então abra o body. `skills/_index.yaml` é fonte de verdade, não conteúdo para carregar por inteiro; rótulo de maturidade nunca sobrepõe `high_performance_eligible`.
...
- **Documentos:** OCR/leitura de autos em PDF (`ocr-autos-pdf`); áudio/vídeo (audiência, depoimento, CFTV) via `npx legalsquad captura`, comando nativo com transcrição **local** para sigilo (skill `captura-midia-av`); geração/edição de peças (Word/PDF); assinatura (`assinatura-peca`)
