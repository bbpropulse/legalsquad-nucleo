...
// init/update) › ~/.legalsquad/motor.json (gravado por install-global: vale
// para todos os projetos da máquina) › prefixos conhecidos do node que está
// rodando › `npm root -g`.
//
...
export function candidatos({
  ...
  // O registro da máquina, gravado pelo install-global (HOME do serviço pode
  // não ser o do usuário: tenta o do node também).
  registros.push(join(home, '.legalsquad', 'motor.json'));
  for (const registro of registros) {
    try {
      const { bin } = JSON.parse(readFileSync(registro, 'utf-8'));
      if (typeof bin === 'string' && bin) lista.push(bin);
    } catch {
...
function main() {
  ...
  if (!bin) {
    ...
    console.error('Instale com: npm install -g github:bbpropulse/legalsquad-nucleo');
