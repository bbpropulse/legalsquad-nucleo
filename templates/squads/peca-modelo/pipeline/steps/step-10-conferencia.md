...
A minuta em `squads/peca-modelo/output/peca-modelo-minuta.md`, o veredito em `squads/peca-modelo/output/revisao.md` e o manifesto do Citation Gate. Este step fecha a entrega FORA do loop de revisão.

## Instructions

### Process

1. Acionar `conferente-modelo` (subagente): grava a versão final revisada em output/peca-modelo-final.md sem reescrever o mérito, e gera ao lado o manifesto `output/peca-modelo-final.md.citation-gate.json` do cartório do run, sem escrevê-lo à mão: `node scripts/squad-state.mjs manifesto-final squads/peca-modelo --peca <peça final> --por conferente-modelo` (uma entrada em `citations[]` por citação da peça, com fonte, `evidence` e verificadores; SHA-256 do arquivo exato; `pendencias_do_profissional[]` dos marcadores de dado, com `procurado_em` e `diligencia` passados em `--pendencias`). Citação da peça sem entrada no cartório recusa o manifesto e volta ao Citation Gate incremental; sem o manifesto, o hook bloqueia a gravação.
