...
1. Acionar `conferente-modelo` (subagente): grava a versão final revisada em output/peca-modelo-final.md sem reescrever o mérito, e ao lado o manifesto `output/peca-modelo-final.md.citation-gate.json` (SHA-256 do arquivo exato; `scope: citacoes_materiais`; **uma entrada em `citations[]` por citação da peça**, com `title` na mesma classe e número que o texto usa, `status: verificada`, `source_url` HTTPS e `consulted_at`), a partir do cartório do run: `node scripts/squad-state.mjs citacoes-pendentes squads/peca-modelo --peca <peça final>` tem de responder `nada-a-verificar`, e as `reaproveitadas` são as entradas do manifesto. Sem o manifesto, o hook bloqueia a gravação.
