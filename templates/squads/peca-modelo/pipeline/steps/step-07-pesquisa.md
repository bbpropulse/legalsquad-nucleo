...
1. Acionar `pesquisador-modelo` (subagente): superiores, vinculantes do tribunal e acervo local sempre; busca externa só se o intake autorizou (ler a resposta no ledger com run-status). Acervo por `npx legalsquad search-acervo --query "<tema ou identificador>" --json` e `Read` dos `.md` devolvidos; nunca `Read` nos `_index.yaml`. Julgado achado no acervo é fonte oficial (a `fonte_url` do frontmatter é o informativo); não baixar o informativo de novo.
