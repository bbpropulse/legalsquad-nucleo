...
export function consolidarCarteira(casosDir, { pastasDeCasos = [] } = {}) {
  const rows = [];
  const skipped = [];
  const fontes = [casosDir, ...pastasDeCasos].filter((d) => existsSync(d));
  if (fontes.length === 0) return { rows, skipped, diretorio_ausente: true };
  const vistos = new Set();
  for (const fonte of fontes) {
    for (const entry of readdirSync(fonte, { withFileTypes: true })) {
      if (!entry.isDirectory() || entry.name.startsWith('_')) continue;
      const rowPath = join(fonte, entry.name, 'carteira-row.json');
      if (!existsSync(rowPath) || vistos.has(rowPath)) continue;
      vistos.add(rowPath);
      try {
...
// Pastas que nunca são casos: o núcleo, o conteúdo ligado do depósito, os
// squads, dependências e o próprio acervo (cujo `casos/` já é a fonte canônica).
const NAO_SAO_CASOS = new Set(['_legalsquad', 'acervo', 'skills', 'squads', 'node_modules', 'dashboard', 'scripts', 'output', '.claude', '.codex', '.Codex', '.git']);

/**
 * Pastas de casos FORA de `acervo/casos/`: no desenho "uma pasta por
 * escritório, casos dentro", o caso vive onde o usuário o abriu
 * (`Processos/<caso>/`, `Clientes/<nome>/<caso>/`), com `carteira-row.json`
 * gravado ali pela skill de carteira. Devolve as pastas-mãe (até dois níveis
 * abaixo da raiz) que contêm ao menos um caso com `carteira-row.json`.
 */
export function pastasDeCasosDaCasa(raiz) {
  const achadas = [];
  const olhar = (dir, nivel) => {
    let entradas;
    try { entradas = readdirSync(dir, { withFileTypes: true }); } catch { return; }
    const subpastas = entradas.filter((e) => e.isDirectory() && !e.name.startsWith('.') && !e.name.startsWith('_') && !NAO_SAO_CASOS.has(e.name));
    if (subpastas.some((e) => existsSync(join(dir, e.name, 'carteira-row.json')))) achadas.push(dir);
    if (nivel < 2) for (const e of subpastas) olhar(join(dir, e.name), nivel + 1);
  };
  olhar(raiz, 0);
  return achadas.filter((d) => d !== join(raiz, 'acervo', 'casos'));
}
...
export function writeCarteira(casosDir, opcoes = {}) {
  const { rows, skipped, diretorio_ausente } = consolidarCarteira(casosDir, opcoes);
...
export function metricasCarteira(casosDir, opcoes = {}) {
  const { rows, skipped, diretorio_ausente } = consolidarCarteira(casosDir, opcoes);
...
if (isMain()) {
  ...
  const pastasDeCasos = pastasDeCasosDaCasa(process.cwd());
  const summary = writeCarteira(casosDir, { pastasDeCasos });
  if (summary.diretorio_ausente) {
    // Fail-closed: sem o diretório não há número verdadeiro a reportar.
    const msg = `CARTEIRA: nenhuma pasta de casos encontrada (${casosDir}, nem pasta de caso com carteira-row.json na casa): nada foi consolidado. Isto NÃO significa carteira vazia.`;
