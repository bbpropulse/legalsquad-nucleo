...
//   node scripts/squad-state.mjs citacoes-pendentes <squad-dir> --peca <caminho real da minuta>
...
function gravidadeDoFix(fix) {
  ...
  return { gravidade, texto: fix.slice(m[0].length).trim(), explicita: true };
}
...
// acesso_falhou) sai das verificadas na hora: contestação recente vence
// confirmação antiga.
const STATUS_VERIFICADA = 'verificada';
const STATUS_CONTESTADA = new Set(['divergente', 'nao_encontrada', 'nao-encontrada', 'acesso_falhou', 'acesso-falhou']);
...
function registrarCitacoes(dir, arquivo, { gate, reviewer }) {
  ...
  const resumo = { verificadas: 0, contestadas: 0, recusadas: [] };
  const quando = now();
  for (const [i, entrada] of entradas.entries()) {
    ...
    if (status === STATUS_VERIFICADA) {
      ...
      ledger.verificadas[chave] = { title, status: STATUS_VERIFICADA, source_url: url, consulted_at: consultada, verificador: reviewer || '', gate, registrada_em: quando };
      resumo.verificadas += 1;
      continue;
    }
    if (STATUS_CONTESTADA.has(status)) {
      delete ledger.verificadas[chave];
      ledger.contestadas[chave] = { title, status, observacao: typeof entrada.observacao === 'string' ? entrada.observacao : '', verificador: reviewer || '', gate, registrada_em: quando };
      resumo.contestadas += 1;
      continue;
    }
    resumo.recusadas.push(`"${title}": status "${entrada.status}" desconhecido (aceitos: verificada, divergente, nao_encontrada, acesso_falhou)`);
  }
 ...
 * as únicas que o verificador precisa abrir) e `contestadas` (as que um
 * verificador derrubou e seguem no texto).
 */
function cmdCitacoesPendentes(dir, flags) {
  ...
  const resultado = {
    ...
    pendentes,
    contestadas,
    acao: pendentes.length ? 'verificar-pendentes' : 'nada-a-verificar',
    detail: pendentes.length
      ? `${pendentes.length} citação(ões) sem veredito nesta versão; ${reaproveitadas.length} já conferida(s) neste run`
      : `todas as ${saida.total} citações do texto já foram conferidas neste run; nenhum verificador a despachar`,
  };
...
function cmdCitacoesStatus(dir) {
  ...
  console.log(JSON.stringify({
    verificadas: Object.values(ledger.verificadas),
...
if (!command || !dir)
  die('uso: squad-state <init|step|checkpoint|complete|fail|review-open|review-verdict|review-status|citacoes-pendentes|citacoes-status|run-status> <squad-dir> [opções]');
