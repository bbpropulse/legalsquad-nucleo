...
//   node scripts/squad-state.mjs review-verdict <squad-dir> --reviewer <id> --verdict APPROVE|REJECT [--fix "<gravidade>: ..."]... [--ajuste "..."]... [--citacoes <json>] [--expect <N>]
...
//   node scripts/squad-state.mjs citacoes-pendentes <squad-dir> --peca <caminho real da minuta> [--confirmacoes N]
...
import { readFileSync, writeFileSync, renameSync, existsSync, appendFileSync, mkdirSync, readdirSync, mkdtempSync, rmSync } from 'node:fs';
...
function combineVerdicts(verdicts) {
  ...
  if (!list.length) {
    return { verdict: 'UNREADABLE', fixes, ajustes, reviewers, unreadable: ['(nenhum veredito recebido)'], rejecting };
  }
  if (unreadable.length) return { verdict: 'UNREADABLE', fixes, ajustes, reviewers, unreadable, rejecting };
  return { verdict: rejecting.length ? 'REJECT' : 'APPROVE', fixes, ajustes, reviewers, unreadable, rejecting };
}
 ...
 */
const PERFIS = {
  completo: { citation_verifiers: null, meta_verifiers: null, max_review_cycles: null, persuasao: true, red_team: true },
  equilibrado: { citation_verifiers: 1, meta_verifiers: 1, max_review_cycles: 2, persuasao: true, red_team: false },
  rapido: { citation_verifiers: 1, meta_verifiers: 1, max_review_cycles: 1, persuasao: false, red_team: false },
};
...
function descreverPerfil(perfil) {
  ...
  const partes = [`${g.citation_verifiers ?? 'N'} verificador(es) de citações por gate`, `${g.meta_verifiers ?? 'N'} avaliador(es) da meta`, `${g.max_review_cycles ?? 'N'} ciclo(s) de revisão`];
...
function tetoDoPerfil(dir, botao, pedido, rotulo) {
...
function cmdRitmo(dir, flags) {
  ...
  if (flags.verificadores !== undefined) {
    ajustes.citation_verifiers = inteiro(flags.verificadores, 'verificadores');
    ajustes.meta_verifiers = ajustes.citation_verifiers;
  }
...
function cmdReviewVerdict(dir, flags) {
  ...
  const expectPedido = flags.expect === undefined ? 1 : Number(flags.expect);
  if (!Number.isInteger(expectPedido) || expectPedido < 1) die('--expect precisa ser inteiro >= 1');
  // Voting acima do ritmo deixaria o ciclo esperando um votante que nunca vem.
  const expect = tetoDoPerfil(dir, gate === 'citacao' ? 'citation_verifiers' : 'meta_verifiers', expectPedido, `votantes esperados no gate "${gate}"`);
  const entry = {
  ...
  return emitDecision(citacoes ? { ...result, gate, citacoes } : { ...result, gate });
}
...
function registrarCitacoes(dir, arquivo, { gate, reviewer }) {
  ...
  const resumo = { verificadas: 0, contestadas: 0, recusadas: [], avisos: [] };
  const quando = now();
  for (const [i, entrada] of entradas.entries()) {
    ...
    if (status === STATUS_VERIFICADA) {
      ...
      const evidencia = { ...((anterior && anterior.evidence) || {}), ...evidenciaDe(entrada, resumo.avisos, title) };
      ...
      const nova = { verificador: reviewer || '', gate, consulted_at: consultada, hash: evidencia.sha256_texto || evidencia.sha256_bytes || null, registrada_em: quando };
      if (!confirmacoes.some((c) => c.verificador === nova.verificador && c.consulted_at === nova.consulted_at)) confirmacoes.push(nova);
      ledger.verificadas[chave] = { title, status: STATUS_VERIFICADA, source_url: url, consulted_at: consultada, evidence: evidencia, verificador: reviewer || '', gate, registrada_em: quando, confirmacoes };
    ...
    resumo.recusadas.push(`"${title}": status "${entrada.status}" desconhecido (aceitos: verificada, divergente, nao_encontrada, acesso_falhou, fonte_mudou)`);
  }
...
function cmdCitacoesPendentes(dir, flags) {
  ...
  const exigidas = tetoDoPerfil(dir, 'citation_verifiers', exigidasPedidas, 'confirmações exigidas por citação');
  const ledger = lerCitacoesDoLedger(dir);
  ...
  const pendentes = saida.descobertas.map((c) => ({ bruto: c.bruto, linha: c.linha, classe: c.classe, numero: c.numero, artigo: c.artigo, diploma: c.diploma, orgao: c.orgao }));
  const pendentesDeConsenso = reaproveitadas
    .filter((v) => contarConfirmacoes(v) < exigidas)
    .map((v) => ({ title: v.title, source_url: v.source_url, confirmacoes: contarConfirmacoes(v), faltam: exigidas - contarConfirmacoes(v), verificadores: (v.confirmacoes || [{ verificador: v.verificador }]).map((c) => c.verificador) }));
  ...
  const resultado = {
    ...
    detail: acao === 'verificar-pendentes'
      ? `${pendentes.length} citação(ões) sem veredito nesta versão; ${reaproveitadas.length} já conferida(s) neste run${pendentesDeConsenso.length ? `; ${pendentesDeConsenso.length} ainda sem ${exigidas} confirmações` : ''}`
      : acao === 'confirmar-consenso'
        ? `todas as ${saida.total} citações do texto já foram conferidas neste run; ${pendentesDeConsenso.length} ainda não soma(m) ${exigidas} confirmações distintas`
        : `todas as ${saida.total} citações do texto já foram conferidas neste run${exigidas > 1 ? `, cada uma com ${exigidas} ou mais confirmações` : ''}; nenhum verificador a despachar`,
  };
...
function cmdCitacoesStatus(dir) {
  ...
  console.log(JSON.stringify({
    resumo: { verificadas: verificadas.length, contestadas: Object.keys(ledger.contestadas).length, por_confirmacoes: porConfirmacoes, com_hash: verificadas.filter((v) => v.evidence && (v.evidence.sha256_texto || v.evidence.sha256_bytes)).length },
    verificadas,
...
function mtimesDoIndiceDosAutos(dir) {
  const caminho = join(dir, 'autos', '_index.yaml');
  if (!existsSync(caminho)) return null;
...
function cmdReabrir(dir, flags) {
  ...
  console.log(JSON.stringify({ runId: novo.runId, reabertura: r.numero, modo: r.modo, pedido: r.pedido, versao_anterior: versao, proxima_versao: versao ? `v${Number(versao.slice(1)) + 1}` : 'v1', checkpoints: Object.keys(novo.checkpoints || {}), ritmo: novo.ritmo || null }, null, 2));
  return null;
}
...
if (!command || !dir)
  die('uso: squad-state <init|ritmo|step|checkpoint|complete|fail|reabrir|review-open|review-verdict|review-status|citacoes-pendentes|citacoes-status|run-status> <squad-dir> [opções]');
