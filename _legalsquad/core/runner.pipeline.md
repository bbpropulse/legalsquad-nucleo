...
6b. **Abra o run** (escritor determinístico). Em vez de montar o JSON à mão, **chame o escritor** a partir da raiz do workspace (`{root}`):
   ...
   - **Autos por referência (uma pasta por escritório, casos dentro).** Se o usuário abriu a IDE na pasta de um caso (uma subpasta da casa com `autos/`), os autos são **os de lá**: não copie. Grave `squads/{name}/caso.json` com `{"pasta": "<pasta do caso, relativa à raiz>", "autos": "<pasta>/autos"}` e rode `node scripts/indexar-autos.mjs "<pasta do caso>"` (e `python3 scripts/autos-para-md.py "<pasta do caso>"`); o hook de redação e os agentes leem o índice de lá. `squads/{name}/autos/` continua valendo para quem prefere copiar.
...
- **Três paradas humanas, com nome — e nenhuma outra.** Um squad de entrega
  ...
  - **`intake`** (coleta, primeiro step): objetivo, prazo, juízo e instância,
    ...
    linguagem de gente: **"Rápido"** (1 verificador de citações por gate, 1
    avaliador da meta, 1 ciclo de revisão, sem gate de persuasão nem red-team)
    · **"Equilibrado"** (1 verificador por gate, 2 ciclos de revisão, persuasão
    em uma passada, sem red-team) · **"Rigoroso"** (o que o squad declara:
    consenso de 3 verificadores, 3 avaliadores, 3 ciclos, persuasão e red-team
    oferecido). Referência medida (run de 16/09/2026, peça de 15 páginas com 39
    ...
    CÓDIGO: `review-open`/`gate-open` rebaixam `--max`, `gate-verdict` rebaixa
    `--expect` e `citacoes-pendentes` rebaixa `--confirmacoes` ao teto do ritmo,
    com aviso no stderr; `persuasao: false` pula o Passo 4.6 e `red_team:
...
4.6 Gate de Sobrevivência ao Resumo (só squad que entrega peça com `reader: juiz` ou `cliente`, e só com o ritmo do run em `persuasao: true` — subagente verificador-persuasao × `meta_verifiers`; a peça tem de sobreviver ao resumo de triagem e ancorar cada tese em Tema; os `fixes` entram no mesmo loop, com gravidade; uma passada e uma reconferência, teto 2)
...
0. **Baixar as fontes oficiais por código, antes de despachar verificador.** Medido no run de 15/09/2026: o verificador só tem `WebFetch`, e o `WebFetch` não abre o SCON do STJ (exige User-Agent de navegador e `Referer`) nem o e-SAJ (devolve um gateway de JavaScript) — nove citações do acórdão foram "acesso_falhou" que não eram. Este passo resolve o acesso com código, uma vez, e deixa a cópia local para todos os verificadores do run:
   ```bash
   node scripts/fonte-oficial.mjs --pesquisa {output/pesquisa-juridica.md} --peca {minuta} --out {pasta do run}/fontes
   ```
   Baixa toda URL oficial (`.jus.br`, `.gov.br`, `.leg.br`) que a pesquisa e a peça citam, guarda a cópia e o texto extraído, e escreve `fontes/INDEX.jsonl` — uma linha por fonte, com `url`, `status`, `motor`, `arquivo`, `texto`, `sha256_texto`/`sha256_bytes` e `baixado_em`. Uma citação do STJ sem URL de inteiro teor resolve-se por `--stj "{citação}"`, que acha registro e data de publicação na própria página de resultados do tribunal. Nunca resolve captcha nem faz login: o que exige um dos dois volta `acesso_falhou` com o motivo, e é isso que o verificador vai dizer.
...
1. **Verificar (subagente isolado), só o que ainda não foi verificado neste run.** Antes de despachar, pergunte ao cartório o que falta:
   ...
   - **Voting no gate FINAL (padrão parallelization-voting).** No último Citation Gate antes da entrega/protocolo (peça que vai ao humano para aprovação final), despache **`citation_verifiers` verificadores independentes em paralelo** (default **3**; lido do `squad.yaml` ou do step) — cada um em contexto fresco, uma única mensagem com N `Task`. **Consenso:** uma citação só é VERIFICADA se a **maioria** confirmar; se **qualquer** verificador marcar NÃO ENCONTRADA/DIVERGENTE, trate como pendência (conservador — risco com sanção real). Em gates intermediários, 1 verificador basta (custo). Não use voting em squads que não produzem peça com citações. **O gate final também é incremental, e a reabertura é do CÓDIGO.** Medido no run de 15/09/2026: reabrir 25 fontes × 3 votantes com o cartório limpo custou ~16 minutos e ~850k tokens para confirmar o que já estava confirmado. A ordem agora é:
     ...
     node scripts/squad-state.mjs gate-verdict squads/{name} --gate citacao \
       --reviewer reabertura --verdict APPROVE|REJECT --citacoes {pasta do run}/citacoes/reabertura.json
     node scripts/squad-state.mjs citacoes-pendentes squads/{name} --peca {peça final} --confirmacoes {citation_verifiers}
     ```
     `--reabrir` rebaixa cada citação com evidência gravada: **hash igual** (ou trecho ainda presente) → `verificada` com verificador `reabertura`; **hash diferente** → `fonte_mudou`, que o cartório trata como contestação; **sem acesso** → `acesso_falhou`. Isso é **uma** confirmação por código, somada às do run: com `citation_verifiers: 3`, a citação conferida na rodada de redação (1) mais a reabertura (2) precisa de **um** votante LLM (3) — e `citacoes-pendentes --confirmacoes 3` diz exatamente quais. **Só vão a LLM** as citações em `pendentes` (nunca conferidas), as `contestadas` (inclusive as `fonte_mudou`), as que voltaram em `sem_hash` (o cartório não tem evidência para comparar) e as de `pendentes_de_consenso`. O resto é confirmado por código, e o chefe diz isso.
...
3. **Loop gerador→verificador — a contagem é do CÓDIGO.** Abra o laço uma vez, no primeiro gate do step, e registre cada veredito. **Não conte ciclos de cabeça:** um Citation Gate que perde a conta ou "esquece" de escalar deixa passar peça com citação não verificada — o risco com sanção real que este gate existe para impedir.
   ...
   ```
   `--max` default **3**. Com voting, passe `--expect N` em cada veredito: o combinador é o mesmo do loop de revisão — **qualquer** REJECT derruba os APPROVEs, o que é exatamente a regra conservadora que este gate pede. **Em cada veredito, leve a tabela do verificador ao cartório com `--citacoes {tabela.json}`**: um JSON no formato do manifesto (`citations[]` com `title` na mesma classe e número que o texto usa, `status` `verificada` / `divergente` / `nao_encontrada` / `acesso_falhou`, `source_url` HTTPS e `consulted_at`), gravado por você a partir da tabela do relatório, sem editorializar. É o que a rodada seguinte reaproveita, e é de onde o conferente tira o manifesto: `citacoes-pendentes --peca {peça final}` com `nada-a-verificar` devolve em `reaproveitadas` exatamente as entradas do `citations[]`. Obedeça a `action` devolvida: `revise` → devolva ao step de redação **apenas** os `fixes` (as citações problemáticas); `advance` → siga; `escalate` (**exit code 3**) → pare e leve ao usuário com a lista de pendências, **sem** finalizar.
4. **Rede determinística (hook).** O hook `verifica-citacoes` (PostToolUse, Write/Edit) bloqueia a gravação de artefato final em `squads/*/output/` enquanto restar qualquer marcador de pendência, faltar o manifesto `<artefato>.citation-gate.json` ao lado (SHA-256 do arquivo exato) ou o manifesto não trouxer **uma entrada em `citations[]` para cada citação material do texto** (mesma classe, mesmo número; a mensagem lista as descobertas com a linha) — garante que o gate não seja "esquecido". **Quem grava o manifesto é o conferente do último step antes da aprovação**, a partir da tabela por citação do verificador (`APROVADO` = todas verificadas): o runner não o inventa. O hook reconhece a peça final pelo nome (`NOMES_DE_PECA`: `contestacao`, `apelacao`, `sentenca`… em qualquer posição, salvo prefixo interno como `analise-`, `resumo-`, `pesquisa-`), pela forma (duas fórmulas de peça em trechos distintos) ou pelo sinal explícito (`output/final/`, marcador, `citation_gate: final`); a minuta escapa por `minuta`/`rascunho`/`draft` no nome ou `citation_gate: draft`.
5. **Narre o rigor — inclusive quando PASSA.** O gate mudo só na falha faz o aluno nunca descobrir o que o produto fez por ele. Uma linha do chefe, com os números do ledger:
   ...
   - No gate final com reabertura por código: `{icon do chefe} Reabri as {N} fontes oficiais e comparei com o que foi citado: {I} idênticas, {M} mudaram, {F} não abriram. Mandei {L} para conferência humana em vez de {N}.`
...
| Citações verificadas neste run (o veredito vale por citação, não por ciclo) | `squads/{name}/review-state.json` (chave `citacoes`) | `review-verdict`/`gate-verdict … --citacoes <tabela.json>` | `citacoes-pendentes --peca <minuta> [--confirmacoes N]` (o que falta conferir e o que falta confirmar), `citacoes-status` |
