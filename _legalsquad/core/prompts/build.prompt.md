...
- **Parada `intake`**: o chefe apresenta a recomendação de `node scripts/cobertura-acervo.mjs .
  --tema "{tema}" --tribunal {sigla} --instancia {1|2|superior}` como veio, e pergunta o escopo da
...
- **Redação Gate — hook `verifica-redacao`, PostToolUse em Write/Edit — seis sinais**:
  ...
  motor; com `skills: []` no `squad.yaml` sai `nao-avaliado`, e agente e step não redefinem o
  ...
  `[XXX]`), `vicios` (tolerância zero a travessão — `—`, ou `–` espaçado — na prosa redigida,
  fora de citação transcrita; densidade de marcas de IA com teto 4 fora de citação — os quatro
...
- **Citation Gate — hook `verifica-citacoes` — o que ele trata como artefato final** (e por
  ...
  grava o manifesto é o agente que promove a minuta a final (o conferente, no squad de peça),
  depois da conferência material pelo `verificador-citacoes` do próprio step; **o pipeline
  ...
  grava manifesto nenhum. Campos do manifesto: `verified_at`/`consulted_at` em ISO 8601 com fuso
  (`2026-07-09T18:00:00-03:00`) e `source_url` em https — o hook valida o formato, então a
...
3. **Onda 3 — os arquivos determinados:** com elenco e topologia congelados, cada `.agent.md` e
   ...
   `pipeline.yaml`. No modo padrão, gere **um de cada vez**, cada um com o cuidado de quem escreve
   o único. Há um modo paralelo, opt-in, descrito logo abaixo.
