...
**`contracted` executa em produção sem confirmação por run** (decisão de 18/09/2026): a supervisão é a revisão humana da peça, que todo run já exige. É o estado de quase todo o catálogo instalado; não é motivo para ligar menos skills nem para descrevê-las como não confiáveis. Bloqueado continua sendo `preview`, `deprecated`, `quarantined`, hard fail estrutural e promoção alegada sem evidência.
...
2. Qual o **polo de atuação** deste squad? (polo ativo: autor/requerente/reclamante/acusação · polo passivo: réu/requerido/reclamada/defesa · terceiro interessado · consultivo/extrajudicial, no vocabulário da área): herde o `Polo predominante` do `company.md` e apenas confirme; se lá estiver `Varia por caso`, pergunte, porque aí é o caso que decide.
...
4. **Só se o tipo de trabalho produzir peça** (peça de 1º grau, recurso, medida de urgência ou tipo de peça exposto pelo catálogo): *"Quer que o chefe do squad ofereça um **red-team** antes de você aprovar a peça? Ele devolve os três ataques mais fortes que a parte contrária faria (um de fato, um de direito, um de forma) e diz quais a peça já antecipa. Custa uma rodada a mais de subagente, alguns minutos, e só roda se você disser sim na hora."* (sim / não). **Default: sim**: peça sujeita a sanção real por erro é onde o ataque simulado mais paga. A resposta vai para `context.red_team` e o Build a honra ao escrever `meta_verifiers` (sim → `3`; não → `1` **explícito**, para o `check-squad` saber que foi escolha e não esquecimento). Para trabalho que não produz peça, **não pergunte** e omita o campo.
...
context:
  ...
  polo: "{polo de atuação deste squad, no vocabulário da área: ativo | passivo | terceiro | consultivo; herdado do Polo predominante de company.md, ou perguntado quando lá está Varia por caso}"
  ...
  red_team: "{sim | não; só quando o trabalho produz peça, omita o campo nos demais. sim arma o contraditor (meta_verifiers: 3); não grava meta_verifiers: 1 explícito}"
