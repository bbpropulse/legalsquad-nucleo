...
## Catalogue Preflight (MANDATORY — before any research)

After giving the session notice below and **before the first browser/web action**, reduce the research purpose to capability terms without case data and run `npx legalsquad search-skills --query "<capability>" --limit 8 --json`. Use only that compact shortlist; never load `skills/_index.yaml` in full. For architecture work, `--include-preview` may expose test sources. Area packages may ship integration manifests (`skills/_*-integration.yaml`); query them only by targeted search when a bridge capability must be resolved — their names come from the installed area catalogue, so discover them on disk, never presume one.

Use the shortlist to avoid researching or proposing a capability that already exists. Treat lifecycle, label and evidence as separate gates: `active` means available, not proven; `pilot` needs explicit opt-in and an active fallback; `preview` is test-only; `deprecated` resolves to its successor; `quarantined` is blocked. Prefer `certified` then `verified` only when `high_performance_eligible: true`; a label with false or missing computed eligibility is not a promotion. A `contracted` capability may be recommended only with its profile contract, guards and human supervision, and must be labelled as lacking full behavioral evidence. Ignore `legacy`, `preview` and `quarantined` in new production designs. Read `risk`, `delivery_type`, `freshness_policy`, `guard_triggers` and `eval_case_ids` from the shortlist before recommending a candidate.

If the squad purpose, reference profile or research topic falls inside a subject matter for which the **installed area package** ships a high-performance protocol in `_legalsquad/core/best-practices/`, read that protocol before researching — discover it through the area's `_catalog.yaml` (or by listing the directory), never by presuming a filename. **When no such file exists** (área não instalada, or the topic has no protocol), note `area_protocol: not_installed` in the `catalog_preflight` and continue, never reconstructing the protocol from memory. When it exists, apply its freshness, official-source, calculation, evidence, Citation Gate and human-review protocol. `preview` files of the area may inform an audit/evolution study, but the output must route production work to canonical released targets from the manifest.

Record a compact `catalog_preflight` note in `pattern-analysis.md`: files read, canonical capabilities already available, lifecycle exclusions, and (when applicable) which area protocol was applied — or `area_protocol: not_installed`.

## Session Management (ALWAYS inform the user)

At the START of every investigation, before any browser action, tell the user:

> "Your browser sessions are saved in `_legalsquad/_browser_profile/`. To clear a platform's session, just delete the JSON file (e.g., `instagram.json`). I'll ask before saving any new session."
...
2. For each URL, the Architect launches a Sherlock subagent in the background using the Task tool and passes the catalogue context
3. The Sherlock subagent completes the catalogue preflight above
4. Each subagent runs independently — one profile per subagent
5. Subagents produce two files each: `raw-content.md` and `pattern-analysis.md`
6. After all subagents complete, the Architect (Design phase) consolidates findings into `consolidated-analysis.md`
7. The consolidated analysis enriches all Phase 3 extraction artifacts
...
Browser sessions are stored as JSON files in `_legalsquad/_browser_profile/`:

- Instagram: `_legalsquad/_browser_profile/instagram.json`
- YouTube: `_legalsquad/_browser_profile/youtube.json`
- Twitter/X: `_legalsquad/_browser_profile/twitter.json`
- LinkedIn: `_legalsquad/_browser_profile/linkedin.json`

**Loading a session (Playwright CLI):**
```bash
npx playwright open --load-storage=_legalsquad/_browser_profile/{platform}.json {url}
```

**Saving a session (Playwright CLI):**
```bash
npx playwright open --save-storage=_legalsquad/_browser_profile/{platform}.json {url}
```
...
Session files are stored in `_legalsquad/_browser_profile/` as JSON files (one per platform). This directory is gitignored and persists between runs.
...
6. **Step 2 — Save the session (quick — user is already logged in):**
   ...
   If yes:
   ```bash
   npx playwright open --save-storage=_legalsquad/_browser_profile/{platform}.json {platform-url}
   ```
...
7. If the user declines saving, inform them:
   "No problem. Remember your cookies are still in `_legalsquad/_browser_profile/` — delete the folder to clear everything."
...
Users can clear all saved browser sessions at any time by deleting the `_legalsquad/_browser_profile/` directory or individual JSON files. After deletion, the next investigation will require a fresh manual login.
